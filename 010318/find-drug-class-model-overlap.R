...
drug.classes.by.data.set <- list()
for(ds in names(fits)) {
  common.drugs.by.data.set[[ds]] <- drug.name.tbl[, drug.name.col]
  drugs.by.data.set[[ds]] <- drug.name.tbl[, drug.name.col]
  drug.classes.by.data.set[[ds]] <- fimm.clusters.of.consistent.drugs
}
...
if(use.subset) {
  ...
  gene.subset <- prioritized.genes$ensg
}
...
if(ds %in% data.sets.to.analyze) {
  ...
  train.drugs[[nxt.indx]] <- drugs.by.data.set[[ds]]
  train.drug.classes[[nxt.indx]] <- drug.classes.by.data.set[[ds]]
...
if(do.downsampling) {
  res.aucs[[1]] <- train.and.test.crossproduct.with.downsampling(gene.sets = gene.sets, drug.name.tbl = drug.name.tbl, train.set.names = train.set.names, train.dss.args = train.dss.args, 
                                         ...
                                         train.common.drugs = train.common.drugs, train.drugs = train.drugs, train.drug.cols = train.drug.cols, train.patient.cols = taclasses.in.patient.cols, 
  ...
  do.model.mean.feature <- FALSE
  do.mean.feature <- FALSE
...
##      colnames(mean.resp) <- gsub("X(.*)\\.(.*)", "\\1-\\2", colnames(mean.resp))
  ...
  res.aucs[["random"]] <- train.and.test.crossproduct(gene.sets = gene.sets, drug.name.tbl = drug.name.tbl, train.set.names = train.set.names, train.dss.args = random.train.dss.args, 
                                         ...
                                         test.genomic.args = test.genomic.args, test.clinical.args = test.clinical.args, test.common.drugs = test.common.drugs,
                                         test.drug.cols = test.drug.cols, test.patient.cols = test.patient.cols, test.response.cols = test.response.cols, 
                                         seed = 1234, use.rf = TRUE, use.cforest = use.cforest, use.svm = FALSE, use.mean = FALSE, num.processes = num.processes, 
                                         alphas = c(0, 1), include.mean.response.as.feature = FALSE,
                                         subtract.mean.response = FALSE, keep.forest = keep.forest)

  res.aucs[["random.class"]] <- train.and.test.crossproduct(gene.sets = gene.sets, drug.name.tbl = drug.name.tbl, train.set.names = train.set.names, train.dss.args = random.train.dss.args, 
                                         train.expr.args = random.train.expr.args, train.genomic.args = train.genomic.args, train.clinical.args = train.clinical.args, 
                                         train.common.drugs = train.common.drugs, train.drugs = train.drug.classes, train.drug.cols = train.drug.cols, train.patient.cols = train.patient.cols, 
                                         train.response.cols = train.response.cols, test.set.names = test.set.names, test.dss.args = test.dss.args, test.expr.args = test.expr.args, 
                                         test.genomic.args = test.genomic.args, test.clinical.args = test.clinical.args, test.common.drugs = test.common.drugs,
                                         test.drug.cols = test.drug.cols, test.patient.cols = test.patient.cols, test.response.cols = test.response.cols, 
                                         seed = 1234, use.rf = TRUE, use.cforest = use.cforest, use.svm = FALSE, use.mean = FALSE, num.processes = num.processes, 
  ...
  res.aucs[["no.transform.class"]] <- train.and.test.crossproduct(gene.sets = gene.sets, drug.name.tbl = drug.name.tbl, train.set.names = train.set.names, train.dss.args = train.dss.args, 
                                         train.expr.args = train.expr.args, train.genomic.args = train.genomic.args, train.clinical.args = train.clinical.args, 
                                         train.common.drugs = train.common.drugs, train.drugs = train.drug.classes, train.drug.cols = train.drug.cols, train.patient.cols = train.patient.cols, 
                                         train.response.cols = train.response.cols, test.set.names = test.set.names, test.dss.args = test.dss.args, test.expr.args = test.expr.args, 
                                         test.genomic.args = test.genomic.args, test.clinical.args = test.clinical.args, test.common.drugs = test.common.drugs,
...
tbl <- subset(tbl.full, train.set == "ohsu")
source("make-prediction-actual-response-plots.R")
