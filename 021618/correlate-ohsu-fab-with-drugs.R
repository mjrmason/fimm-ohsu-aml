...
colnames(ohsu.drc) <- gsub("X(.*)\\.(.*)", "\\1-\\2", colnames(ohsu.drc))
common <- intersect(colnames(ohsu.drc), rownames(ohsu.metadata))
drug.fab <- as.data.frame(t(ohsu.drc[, common]))
drug.fab$fab <- ohsu.metadata[common, "fab"]

colnames(drug.fab) <- gsub("-", ".", colnames(drug.fab))
colnames(drug.fab) <- gsub(" ", ".", colnames(drug.fab))

drugs <- colnames(drug.fab)
drugs <- drugs[(drugs != "fab")]
data <- drug.fab
...
for(drug in drugs) {
  form <- as.formula(paste0(drug, " ~ ", "fab"))
  flag <- !is.na(data[, drug]) & !is.na(data$fab)
  kw <- tryCatch({kruskal.test(form, data = data[flag, ])}, error = function(e) { data.frame(p.value = NA) })
  show.plot <- !is.na(kw$p.value) && (kw$p.value < 0.05)
  show.plot <- show.plot | (drug %in% c("Trametinib", "Venetoclax"))
  if(show.plot) {
    cat(paste0(drug, " is assocated with FAB: p = ", kw$p.value, "\n"))
    pdf(paste0(drug, "-vs-fab-boxplot.pdf"))
##    boxplot(form, data = data[flag, ])
    g <- ggplot(data = data[flag,], aes_string(x = "fab", y = drug))
    g <- g + geom_boxplot()
    g <- g + geom_point()
    g <- g + ggtitle(paste0("Kruskal p = ", kw$p.value))
    print(g)
    d <- dev.off()
  }
}