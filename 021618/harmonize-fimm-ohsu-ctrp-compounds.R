...
cat("Harmonize FIMM, OHSU, and CTRP compounds:\n")
...
cat("Calculating all pairwise Tanimoto distances between CTRP drug fingerprints and OHSU/FIMM drug fingerprints\n")
...
flag <- struct.mapping$ctrp.smiles %in% ctrp.compounds$cpd_smiles
if(all(flag)) {
...
grand.map <- merge(ctrp.compounds, struct.mapping, by.x = "cpd_smiles", by.y = "ctrp.smiles", all = FALSE)
grand.map <- merge(grand.map, ohsu.fimm.drugs, by.x = "ohsu.smiles", by.y = "smiles", all = FALSE)

## Save the mapping of CTRP to FIMM/OHSU compounds in the folder
## FIMM_BEAT AML Collaboration/Files/Analyses/fimm-ohsu-ctrp (syn12180268)
parentId <- "syn12180268"
path <- paste0("fimm-ohsu-ctrp-drug-map.tsv")
...
cat("Successfully created ctrp-fimm-ohsu drug map.\n")