...
## These are the original fits before being restricting to common concentration ranges
data.set.orig.fit.synIds <- list("ohsu" = "syn11470803", "fimm" = "syn11471586")
data.set.orig.fit.files <- c("ohsu.l4.and.ll4.fits.112017.tsv", "fimm.l4.and.ll4.fits.112017.tsv")

## These are the original expression matrices before combat
data.set.orig.expr.synIds <- list("ohsu" = "syn10083723", "fimm" = "syn11288849")
data.set.orig.expr.files <- list("ohsu" = "ohsu.expr.tsv", "fimm" = "fimm-log-cpm-gene-expr.tsv")
...
fits <- llply(data.set.orig.fit.synIds, .parallel = FALSE,
              .fun = function(synId) {
...
exprs <- llply(data.set.orig.expr.synIds, .parallel = FALSE,
              .fun = function(synId) {
                       obj <- synGet(synId, downloadFile = TRUE)
                       file <- getFileLocation(obj)
                       read.table(file, sep="\t", header=TRUE, as.is=TRUE, comment.char="", quote="\"")
