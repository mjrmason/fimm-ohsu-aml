...
do.pca <- function(mat, mean.response, main.prefix, scale = "none", center = FALSE, pca.methods = c("nipals", "bpca", "ppca", "svdImpute")) {
  nPcs <- min(20, min(nrow(mat), ncol(mat)))
  pc.res <- do.pca_(t(mat), nPcs = nPcs, scale = scale, center = center, pca.methods = pca.methods)
...
impute.pca <- function(mat, mean.response, main.prefix, impute.row = TRUE) {
  ...
  imps <- impute.pca_(mat, mice.imputation.methods)
  print(names(imps))
