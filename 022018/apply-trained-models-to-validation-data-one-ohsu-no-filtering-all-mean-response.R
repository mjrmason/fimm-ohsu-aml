...
tbl$pt.response <- factor(tbl$pt.response, levels = c("random", "no.transform", "subtract.mean", "mean.feature", "unpenalized.mean.feature", "mean.feature.only", "model.mean.feature"))
...
baz <- subset(foo, response %in% c("model.mean.feature", "mean.feature", "unpenalized.mean.feature", "mean.feature.only"))
...
pt.responses1 <- c("mean.feature.only")
pt.responses2 <- c("unpenalized.mean.feature")

pt.responses <- c("no.transform", "mean.feature.only", "unpenalized.mean.feature")
    ...
    for(gset in c("gene")) {
      for(met in c("pearson", "spearman")) {
        ...
        for(indx in 1:length(pt.responses1)) {
          ...
          sub1 <- subset(tbl, metric == met & response == resp & gene.set == gset & train.set == "ohsu" & test.set == "fimm" & model == mdl & pt.response == pt.response1)
          sub2 <- subset(tbl, metric == met & response == resp & gene.set == gset & train.set == "ohsu" & test.set == "fimm" & model == mdl & pt.response == pt.response2)
          ...
          for(ds in c("train", "test")) {  
            ...
            g <- g + ylab(paste("Drug/mean response", capwords(met), "correlation (", capwords(ds), "data", ")", sep=" "))
            g <- g + ggtitle(paste0(resp, " ", capwords(met), " ", mdl, " ", gset))
            pdf(paste(file.prefix, mdl, resp, met, gset, pt.response1, "vs", pt.response2, "volcano.pdf", sep="-"))
            print(g)
            d <- dev.off()
          }
          g <- ggplot(data = sub)
          g <- g + geom_point(aes(x = val.1, y = val.2))
          g <- g + xlab(paste(pt.response1, met, "correlation", sep=" "))
          g <- g + ylab(paste(pt.response2, met, "correlation", sep=" "))
          ...
          if(any(flag)) {
            labels <- sub[flag,]
            g <- g + geom_text(data = labels, aes(x = val.1, y = val.2, label = train.drug))
          }
          g <- g + ggtitle(paste0(resp, " ", capwords(met), " ", mdl, " ", gset))
          pdf(paste(file.prefix, mdl, resp, met, gset, pt.response1, "vs", pt.response2, "comparison.pdf", sep="-"))
          print(g)
          d <- dev.off()
          ...
          g <- g + ggtitle(paste0(resp, " ", capwords(met), " ", mdl, " ", gset))
          pdf(paste(file.prefix, mdl, resp, met, gset, pt.response1, "vs", pt.response2, "test-mean-response-correlation.pdf", sep="-"))
          print(g)
          d <- dev.off()
          ...
          g <- g + ggtitle(paste0(resp, " ", capwords(met), " ", mdl, " ", gset))
          pdf(paste(file.prefix, mdl, resp, met, gset, pt.response1, "vs", pt.response2, "train-mean-response-correlation.pdf", sep="-"))
          print(g)
          d <- dev.off()
        }
      }
