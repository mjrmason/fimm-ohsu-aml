...
source("ctrp-sanger-fimm-ohsu-setup-022218.R")
...
heatmaps <-
  llply(data.sets,
        .fun = function(ds) {
                 ...
                 hm <- plot.drug.heatmap.no.anno(orig.scaled.drcs[[ds]], show.col.names = FALSE, column_title = title)
               })
...
corrplots <-
  llply(data.sets,
        .fun = function(ds) {
                 title <- toupper(ds)
                 M <- cor(t(orig.scaled.drcs[[ds]]), use = "pairwise.complete.obs")
                 ## order = "hclust" breaks when there are NAs
                 ## corrplot(M, type = "lower", order = "hclust", tl.col = "black", tl.srt = 45, title = title)
                 ## mar <- c(5, 4, 4, 2) + 0.1)
