...
for(gset in gsets) {
  cat(paste0("DEBUG: Doing gset: ", gset, "\n"))
...
##  models <- c("rf")
  ...
  models <- c("ridge", "lasso")
##  models <- c("ridge", models[!(models %in% c("ridge", "rf"))])
##  models <- c("ridge")
##  models <- c("rf")
##  models <- c("rf", "crf")
##  models <- models[models %in% tbl.gset$model]
  models <- unique(tbl.gset$model)[unique(tbl.gset$model) %in% models]
  for(mdl in models) {
    cat(paste0("DEBUG: Doing model: ", mdl, "\n"))
    tbl.model <- subset(tbl.gset, model == mdl)
    responses <- unique(as.character(tbl.model$pt.response))
##    responses <- "no.transform"
    responses <- c("unpenalized.mean.feature", "random", "mean.feature", "no.transform", "model.mean.feature", responses[!(responses %in% c("mean.feature", "random", "no.transform", "model.mean.feature"))])
##    responses <- c("random", "mean.feature")
    ...
    if(gset == "gene") {
      ...
      for(gs.nm in pthwys[pthwys %in% c("go.BP", "kegg")]) {
        cat(paste0("DEBUG: Doing gs.nm: ", gs.nm, "\n"))
...
cat(paste0("DEBUG: Calculating expected overlap for ", gs.nm, " and ", mdl, "\n"))
...
cat(paste0("DEBUG: Done calculating expected overlap for ", gs.nm, " and ", mdl, "\n"))
print(head(names(expected.fracs[[gs.nm]])))
print(head(expected.fracs[[gs.nm]][[list.sizes[1]]]))
      }
    }
    cat("DEBUG: Done with gset gene\n")
    ...
    for(resp in responses) {
      ...
      cat(paste0("DEBUG: Doing response for combo: ", paste(c(gset, mdl, resp), collapse=" "), "\n"))
...
cat(paste0("DEBUG: Calling plot.overlap.of.pathways: ", gs.nm, "\n"))
...
##          plot.overlap.of.pathways(tbl.resp, tbl.sub, universe = universe, transform = "abs.drop.zero", list.sizes = list.sizes, ofile.prefix = ofile, main = main, pathway.set = ensg.to.pathway.list[[gs.nm]])
cat("DEBUG: Done calling plot.overlap.of.pathways\n")
        }
        }
      }
      cat("DEBUG: Done with gset gene2\n")
...
##          save(list.sizes, file="list.sizes.Rd") 
      ...
      cat(paste0("DEBUG: About to do feature-level: ", gset, "\n"))
...
cat("DEBUG: Saving image in foo.R\n")
save.image(rdata.file)
cat("DEBUG: Done saving image in foo.R\n")
  ...
  gs <- 
    dlply(tmp, .variables = c("FIMM_DRUG_NAME"), .parallel = FALSE,
            .fun = function(df) {
                     ...
                     g1 <- g1 + xlab("Number of Features in Venetoclax Ridge Model\n(Ordered by Absolute Value)")
                     g1 <- g1 + ylab("Fraction of Overlapping Features")
                     g1 <- g1 + geom_line(aes(x = top, y = frac.expected, colour = frac.expected.color), linetype = "dashed")
                     g1 <- g1 + theme(text = element_text(size = 20), legend.position = "top", legend.justification = "center")
...
cat("DEBUG: Done with clean-foo.R\n")