...
doGSVA <- function(expr, clin, analysis.name, to.plot=c(), ylabels=c(), kras.status.field="kras", exclude.nras.mutants = TRUE) {
    ...
    for(i in 1:length(bindeaGsets)) {
        cat(paste0("Bindea set: ", names(bindeaGsets)[i], "\n"))
        if(!(names(bindeaGsets)[i] %in% to.plot)) { next }
        cat(paste0("Analyzing Bindea set: ", names(bindeaGsets)[i], "\n"))        
        ...
        genes <- paste(sort(as.character(genes)), collapse=" ")
        missing.genes <- paste(sort(as.character(missing.genes)), collapse=" ")        
                                                                                ...
                                                                                .fun = function(prefix) {
                                                                                    llply(kras.states, .parallel = FALSE,
                                                                                          .fun = function(genotype) {
                                                                                              ...
                                                                                              ## effect size = x - CMS2 MT
                                                                                              res <- wilcox.test(x = gene.expr[flag2], y = gene.expr[flag1], paired = FALSE, conf.int = TRUE, conf.level = 0.95)
                                                                                              dir <- sign(mean(gene.expr[flag1], na.rm=TRUE) - mean(gene.expr[flag2], na.rm=TRUE))
                                                                                              p <- res$p.value
                                                                                              w <- res$statistic
                                                                                              est <- as.numeric(res$estimate)
                                                                                              est.lb <- res$conf.int[1]
                                                                                              est.ub <- res$conf.int[2]
                                                                                              ret <- c(p, est, est.lb, est.ub)
                                                                                              names(ret) <- c("pval", "estimate", "estimate.lb", "estimate.ub")
...
doKrasEffectSizeAnalysis <- function(es.arg, clin.arg, to.plot=c(), kras.status.field="kras", kras.states=c("MT","WT"), num.bootstraps = 1000, seed = 1234) {
    ...
    kras.effect <- function(data, indices) {
        d <- data[indices,]
        ## Report WT - MT
        mean(d$expr[d$label == kras.states[2]]) - mean(d$expr[d$label == kras.states[1]])
    }
    
    tests <- ldply(genes, .parallel = TRUE,
                   .fun = function(gene) {
                       
                       gene.expr <- es[gene,,drop=T]
                       df <- data.frame(expr = gene.expr, label = kras.label)
                       set.seed(seed)
                       b <- boot(df, statistic = kras.effect, strata = df$label, R = num.bootstraps)
                       bci <- boot.ci(b, type = "bca")
                       low <- bci$bca[4]
                       high <- bci$bca[5]
                       mle <- mean(df$expr[df$label == kras.states[2]]) - mean(df$expr[df$label == kras.states[1]])
                       ret.gene <- data.frame(gene.set = gene, low = low, mle = mle, high = high)
                       ret.gene
                   })
...
doKrasCMSEffectSizeAnalysis <- function(es.arg, clin.arg, to.plot=c(), kras.status.field="kras", kras.states=c("MT","WT"), num.bootstraps = 1000, seed = 1234) {
    ...
    kras.label <- sapply(kras, function(x) ifelse(x==1, kras.states[1], kras.states[2]))
    cms <- as.character(clin$cms_label)
    labels <- unlist(apply(cbind(cms, kras.label), 1, function(row) paste0(row[1], row[2])))
    all.labels <- unique(labels)
    base.line <- "CMS2MT"
    all.labels <- sort(all.labels[all.labels != base.line])
    if(exclude.no.lbl) {
        all.labels <- all.labels[!grepl(all.labels, pattern="NOLBL")]
    }
    
    kras.cms.effect <- function(data, indices) {
        d <- data[indices,]
        lst <- list()
        for(label in all.labels) {
            lst[[label]] <- mean(d$expr[d$label == label]) - mean(d$expr[d$label == base.line])
        }
        vec <- unlist(lst)
        names(vec) <- names(lst)
        vec
    ...
    tests <- ldply(genes, .parallel = TRUE,
                   .fun = function(gene) {
                       ...
                       gene.expr <- es[gene,,drop=T]
                       df <- data.frame(expr = gene.expr, label = labels)
                       set.seed(seed)
                       b <- boot(df, statistic = kras.cms.effect, strata = df$label, R = num.bootstraps)
                       ret <- unlist(llply(1:length(all.labels), .fun = function(i) {
                           bci <- boot.ci(b, type = "bca", index = i)
                           low <- bci$bca[4]
                           high <- bci$bca[5]
                           mle <- mean(df$expr[df$label == all.labels[i]]) - mean(df$expr[df$label == base.line])
                           vec <- c(low, mle, high)
                           names(vec) <- c("low", "mle", "high")
                           names(vec) <- paste0(all.labels[i], ".", names(vec))
                           vec
                       }))
                       ret.gene <- c(gene, ret)
                       names(ret.gene) <- c("gene.set", names(ret))
                       ret.gene
                   })
...
plotKrasCMSAnalysis <- function(es.arg, clin.arg, kras.cms.tbl, to.plot=c(), ylabels=c(), analysis.name, kras.status.field="kras", kras.states=c("MT","WT"), plot.adjusted.pvalues = FALSE, plot.pvals.as.stars = FALSE, main = NULL, highlight.subgroups = FALSE) {
    ...
    flag <- to.plot %in% rownames(es)
    to.plot <- to.plot[flag]
    ylabels <- ylabels[flag]

    indices <- 1:length(to.plot)
    names(indices) <- to.plot
    ret.plots <- llply(indices, function(sti){
...
##        p <- p + geom_jitter()
        ...
        if(use.ggplot.2.2.1.limit.code) { 
            ...
            ylimits <- ggplot_build(p)$layout$panel_params[[1]]$y.range
        }
...
plotKrasCMSWithinCMSAnalysis <- function(es.arg, clin.arg, kras.cms.within.cms.tbl, to.plot=c(), ylabels=c(), analysis.name, kras.status.field="kras", kras.states=c("MT","WT"), plot.adjusted.pvalues = FALSE, plot.pvals.as.stars = FALSE, main = NULL) {
    ...
    indices <- 1:length(to.plot)
    names(indices) <- to.plot
    ret.plots <- lapply(indices, function(sti){
...
plotCMSAnalysis <- function(es.arg, clin.arg, cms.tbl, to.plot=c(), ylabels=c(), analysis.name, kras.status.field="kras", kras.states=c("MT","WT"), plot.adjusted.pvalues = FALSE, plot.pvals.as.stars = FALSE, main = NULL, highlight.subgroups = FALSE) {
...
doForestAnalysis <- function(es.arg, clin.arg, analysis.name, to.plot=c(), ylabels=c(), kras.status.field="kras", kras.states=c("MT","WT"), num.permutations = 0, calc.fdr.with.bh = FALSE, calc.fdr = FALSE, seed = 1234, plot.adjusted.pvalues = FALSE, main = NULL) {
    ...
    ordered.cms <- c("CMS4", "CMS3", "CMS2", "CMS1", "NOLBL")
    ## Make CMS2 the base line
    ordered.cms <- c("CMS2", "CMS1", "CMS3", "CMS4", "NOLBL")
    cms2.last.levels <- ordered.cms[ordered.cms %in% unique.cms]
    cms2.last.levels.no.lbl <- cms2.last.levels[cms2.last.levels != "NOLBL"]
    ...
    ## KRAS MT is the baseline
    kras.col <- paste0("kras", kras.states[2])
    ...
    indices <- 1:length(genes)
    names(indices) <- genes
    tests <- llply(indices, .parallel = FALSE,
                   .fun = function(sti) {
                       ...
                       ## Make KRAS MT the base line
                       df <- data.frame(expr=esn.st, CMS=factor(cms, levels=cms2.last.levels), KRAS=factor(kras.label, levels=(kras.states)), site=site.factor)
...
doKrasAnalysis <- function(es.arg, clin.arg, to.plot=c(), kras.status.field="kras", kras.states=c("MT","WT"), num.permutations = 1000, calc.fdr.with.bh = FALSE, calc.fdr = TRUE, seed = 1234) {
    ...
    if( (calc.fdr.with.bh == TRUE) || (calc.fdr == TRUE) || (num.permutations > 0) ) {
        cat("Multiple-testing correction not implemented in doKrasAnalysis\n")
        q(status=-1)
    }
    ...
    tests <- ldply(genes, .parallel = FALSE,
                   .fun = function(gene) {
                       x <- es[gene,,drop=T]
                       ## Performm Mann Whitney U (Wilcoxon rank sum test)
                       ## and return a 95% confidence interval
                       ## Estimate of differences will be of x[kras==0] - x[kras==1]
                       res <- wilcox.test(x[kras==0], x[kras==1], paired = FALSE, conf.int = TRUE, conf.level = 0.95)
                       ## res <- wilcox.test(x ~ kras, paired = FALSE, conf.int = TRUE, conf.level = 0.95)
                       p <- res$p.value
                       ## b <- (mean(x[kras == 1]) - mean(x[kras == 0])) > 0
                       est <- as.numeric(res$estimate)
                       est.lb <- res$conf.int[1]
                       est.ub <- res$conf.int[2]
                       vec <- c(gene, p, est, est.lb, est.ub)
                       names(vec) <- c("gene.set", "pval", "estimate", "estimate.lb", "estimate.ub")
                       vec
                   })
    rownames(tests) <- tests$gene.set
...
plotOverlapOfExpressedGeneSets <- function(expr.m, all.gene.sets, gene.set.names, main = NULL) { 
    ...
    for(i in 1:length(gene.sets)) {
        for(j in i:length(gene.sets)) {
            intersection <- intersect(gene.sets[[i]], gene.sets[[j]])
            m[i,j] <- length(intersection)
            if((i != j) && (length(intersection) > 0)) {
                str <- ""
                if(!is.null(main)) { str <- paste0(main, ": ") }
                cat(paste0(str, "overlap between ", names(gene.sets)[i], " and ", names(gene.sets)[j], " = ", paste(intersection, collapse=","), "\n"))
            }
        }
    }
...
plot.stacked.effect.size.figures <- function(tbl1, tbl2, tbl1.mle.col, tbl1.lb.col, tbl1.up.col, tbl2.mle.col, tbl2.lb.col, tbl2.ub.col, main1, main2, text.size = NULL, switch.sign = TRUE) {

    bar.width <- 0.2
    num.x.labels <- length(unique(tbl1$gene.set))
    if(num.x.labels > 10) {
        bar.width <- 0.5
    }

    ## Need to temporarily switch sign to get WT - MT, in accord
    ## with multivariate.  When we re-run we will compute the statistics
    ## in that direction
    if(switch.sign) {
        tbl1[, tbl1.mle.col] <- - as.numeric(tbl1[, tbl1.mle.col])
        tbl1[, tbl1.lb.col] <- - as.numeric(tbl1[, tbl1.lb.col])
        tbl1[, tbl1.ub.col] <- - as.numeric(tbl1[, tbl1.ub.col])                
        tbl2[, tbl2.mle.col] <- - as.numeric(tbl2[, tbl2.mle.col])
        tbl2[, tbl2.lb.col] <- - as.numeric(tbl2[, tbl2.lb.col])
        tbl2[, tbl2.ub.col] <- - as.numeric(tbl2[, tbl2.ub.col])                
    }
    
    g1 <- ggplot(data = tbl1)
    g1 <- g1 + geom_point(aes_string(x = "gene.set", y = tbl1.mle.col))
    g1 <- g1 + geom_errorbar(aes_string(x = "gene.set", ymin = tbl1.lb.col, ymax = tbl1.ub.col), width = bar.width)
    ...
    ## gr1 <- ggplotGrob(g1)
    g1 <- g1 + geom_hline(linetype = "dashed", yintercept = 0)
    
    g2 <- ggplot(data = tbl2)
    g2 <- g2 + geom_point(aes_string(x = "gene.set", y = tbl2.mle.col))
    g2 <- g2 + geom_errorbar(aes_string(x = "gene.set", ymin = tbl2.lb.col, ymax = tbl2.ub.col), width = bar.width)
    ...
    g2 <- g2 + geom_hline(linetype = "dashed", yintercept = 0)
...
plot.stacked.faceted.effect.size.figures <- function(tbl1, tbl2, tbl1.mle.col, tbl1.lb.col, tbl1.up.col, tbl2.mle.col, tbl2.lb.col, tbl2.ub.col, main1, main2, text.size = NULL) {

    ## Order x axis by factor, then by cms x kras (leave a padding)
    ...
    dx <- dx / tot
    offset <- offset / tot
    mp[["CMS1MT"]] <- offset + 1 * dx
    mp[["CMS1WT"]] <- offset + 2 * dx
    mp[["CMS2WT"]] <- offset + 3 * dx
    mp[["CMS3MT"]] <- offset + 4 * dx
    mp[["CMS3WT"]] <- offset + 5 * dx
    mp[["CMS4MT"]] <- offset + 6 * dx
    mp[["CMS4WT"]] <- offset + 7 * dx

    tbl1$label <- unlist(apply(tbl1[, c("gene.set", "cms", "kras")], 1,
                               function(row) ifelse((row[2] == "CMS3") && (row[3] == "MT"), row[1], "")))
    tbl1$x <- as.numeric(tbl1$gene.set)
    tbl1$x <- unlist(apply(tbl1[, c("x", "cms", "kras")], 1,
                           function(row) as.numeric(row[1]) + as.numeric(mp[[paste0(row[2], row[3])]])))
    tbl1[,tbl1.mle.col] <- as.numeric(tbl1[,tbl1.mle.col])
    tbl1[,tbl1.lb.col] <- as.numeric(tbl1[,tbl1.lb.col])
    tbl1[,tbl1.ub.col] <- as.numeric(tbl1[,tbl1.ub.col])    
    
    tbl2$label <- unlist(apply(tbl2[, c("gene.set", "cms", "kras")], 1,
                               function(row) ifelse((row[2] == "CMS3") && (row[3] == "MT"), row[1], "")))
    tbl2$x <- as.numeric(tbl2$gene.set)
    tbl2$x <- unlist(apply(tbl2[, c("x", "cms", "kras")], 1,
                           function(row) as.numeric(row[1]) + as.numeric(mp[[paste0(row[2], row[3])]])))
    tbl2[,tbl1.mle.col] <- as.numeric(tbl2[,tbl2.mle.col])
    tbl2[,tbl1.lb.col] <- as.numeric(tbl2[,tbl2.lb.col])
    tbl2[,tbl1.ub.col] <- as.numeric(tbl2[,tbl2.ub.col])    

    colnames(tbl1)[colnames(tbl1) == "cms"] <- "CMS"
    colnames(tbl1)[colnames(tbl1) == "kras"] <- "KRAS"    
    colnames(tbl2)[colnames(tbl2) == "cms"] <- "CMS"
    colnames(tbl2)[colnames(tbl2) == "kras"] <- "KRAS"    
    
    g1 <- ggplot(data = tbl1, aes(colour = CMS, linetype = KRAS))
    g1 <- g1 + geom_point(aes_string(x = "x", y = tbl1.mle.col))
    g1 <- g1 + geom_errorbar(aes_string(x = "x", ymin = tbl1.lb.col, ymax = tbl1.ub.col))
    g1 <- g1 + theme(axis.title.x=element_blank(), axis.text.x=element_text(angle = 45, hjust = 1), axis.ticks.x=element_blank())
    if(!is.null(text.size)) {
        g1 <- g1 + theme(axis.text.x=element_text(angle = 45, hjust = 1, size = text.size))
    }
    ...
    g1 <- g1 + theme(axis.title.x=element_blank(), axis.text.x=element_blank())
    ## gr1 <- ggplotGrob(g1)
    g1 <- g1 + scale_x_continuous(breaks = tbl1$x, labels = tbl1$label)
    g1 <- g1 + geom_hline(linetype = "dashed", yintercept = 0)
    
    g2 <- ggplot(data = tbl2, aes(colour = CMS, linetype = KRAS))
    g2 <- g2 + geom_point(aes_string(x = "x", y = tbl2.mle.col))
    g2 <- g2 + geom_errorbar(aes_string(x = "x", ymin = tbl2.lb.col, ymax = tbl2.ub.col))
    g2 <- g2 + theme(axis.title.x=element_blank(), axis.text.x=element_text(angle = 45, hjust = 1), axis.ticks.x=element_blank())
    if(!is.null(text.size)) {
        g2 <- g2 + theme(axis.text.x=element_text(angle = 45, hjust = 1, size = text.size))
    }
    g2 <- g2 + ylab("Relative\nEnrichment Score")
    g2 <- g2 + ggtitle(main2) + theme(plot.title = element_text(hjust = 0.5))
    g2 <- g2 + scale_x_continuous(breaks = tbl2$x, labels = tbl2$label)
    g2 <- g2 + geom_hline(linetype = "dashed", yintercept = 0)
    ...
    gr1.heat <- gtable::gtable_filter(gr1, "guide-box")
    ...
    gr1$heights[which(gr1$layout$name == "panel")] <- unit(5, units="cm")
    gr2$heights[which(gr2$layout$name == "panel")] <- unit(5, units="cm")
    gr1$widths[which(gr1$layout$name == "panel")] <- unit(1, units="cm")
    gr2$widths[which(gr2$layout$name == "panel")] <- unit(1, units="cm")
    ## Align the x axis (which may be offset because of the different
    ## sizes of the y labels)
    gr1$widths[which(gr1$layout$name == "axis-l")] <- unit(1, units="cm")
    gr2$widths[which(gr2$layout$name == "axis-l")] <- unit(1, units="cm")    
    ##    grid.arrange(gr1, gr2)
    return(list(gr1, gr2, gr1.heat))    
}
