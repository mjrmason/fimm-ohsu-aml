...
m <- drug.map[, c("OHSU_DRUG_NAME", "FIMM_DRUG_NAME")]
rownames(m) <- m$FIMM_DRUG_NAME
ohsu.drugs.of.interest <- as.character(m[drugs.of.interest, "OHSU_DRUG_NAME"])
ds <- "ohsu"
ohsu.drugs.of.interest <- ohsu.drugs.of.interest[(ohsu.drugs.of.interest %in% rownames(orig.scaled.drcs[[ds]]))]

make.drug.correlation.plots_(orig.scaled.drcs, file.prefix = "fimm-ohsu-drugs-of-interest-correlations", 
                             multiple.plots.in.one.figure = FALSE, common.drugs = ohsu.drugs.of.interest, drugs.to.label = ohsu.drugs.of.interest) 