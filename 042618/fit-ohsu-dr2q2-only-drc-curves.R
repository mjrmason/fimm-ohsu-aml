...
cat("Use DRC to fit L.4 (4-parameter logistic) and LL.4 (4-paramter log logistic) functions to OHSU data (DR2 Q2 only).\n")
...
## Limit to patients that are _only_ in DR2 Q2.  Note that the expression file is limited to samples from such patients.
## ohsu.dr2q2.only.expr.tsv
synId <- "syn12176648"
obj <- synGet(synId, downloadFile=TRUE)
expr.file <- getFileLocation(obj)
expr <- read.table(expr.file, sep = "\t", header=TRUE)
colnames(expr) <- gsub("\\.", "-", gsub("^X", "", colnames(expr)))
...
patients.to.keep <- subset(rnaseq.sample.summary.tbl, SeqID %in% colnames(expr))$PatientID
drug.data.long <- subset(drug.data.long, patient_id %in% patients.to.keep)
...
output.file <- paste0("ohsu.dr2q2.only.l4.and.ll4.fits.042618.tsv")
...
fits <- merge(fits, rnaseq.sample.summary.tbl, by.x = "lab_id", by.y = "Original_LabID")
...
uniq.conc.orig <- unique(fits[,c("inhibitor", "min.conc.w.outliers.nM", "max.conc.w.outliers.nM")])
cat("Note that the original ranges (before dropping any points) are the same for all of the drugs (modulo rounding errors)\n")
print(uniq.conc.orig[my.dup(uniq.conc.orig$inhibitor),])
uniq.conc <- unique(fits[!fits$any.exclude.ll4,c("inhibitor", "min.conc.nM", "max.conc.nM")])
uniq.conc <- uniq.conc[order(uniq.conc$inhibitor),]
print(uniq.conc[my.dup(uniq.conc$inhibitor),])


source("fimm-ohsu-setup-040218.R")
source("../common/restrict-concentration-to-common-range.R")

dummy.drug.map <- data.frame("inhibitor" = unique(fits$inhibitor))
dummy.drug.map.drug.id.cols <- list("ohsu" = "inhibitor")
ret <- determine.drug.ranges(list("ohsu" = fits), screen.id.cols, data.set.drug.id.cols, 
                             dummy.drug.map, dummy.drug.map.drug.id.cols, shared.conc.postfix = ".auc.conc") 

print(uniq.conc[my.dup(uniq.conc$inhibitor),])
subset(ret$shared.drug.ranges[["LL.4"]][, c("inhibitor", "min.conc", "max.conc")], inhibitor %in% uniq.conc[my.dup(uniq.conc$inhibitor),"inhibitor"])
