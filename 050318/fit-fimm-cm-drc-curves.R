...
for(drug.id in drugs.to.exclude) {
  pdf(paste0("control-", drug.id, "-distribution.pdf"))
...
if(length(pts) != 1) {
  stop("Was only expecting one patient with LOWn/HIGHn controls\n")
}
...
## The ‘Sometimes’ tag comes because in the initial days students would sometimes skip adding CTG in these wells because they are blank anyway. The FO4 layouts however should be more consistent.
...
for(ctrl in names(exclude)) {
  for(var in names(exclude[[ctrl]])) {
    ...
    cat(paste0("Excluding the following ", var, " because of ", ctrl, " (n = ", length(which(flag)), 
               " samples): ", paste(exclude[[ctrl]][[var]], collapse=", "), "\n"))
...
lst <- list("Response at min conc" = responses.at.min.conc, "HIGH" = high.responses, "DMSO" = dmso.responses)
...
lst <- list("Response at min conc (shifted)" = shifted.responses.at.min.conc, "LOW" = low.responses, "BzCl" = bzcl.responses)
indices <- 1:length(lst)
names(indices) <- names(lst)
...
max.conc.mean <- mean(lst[["BzCl"]])
max.conc.sd <- sd(lst[["BzCl"]])
...
drug.data.long <- annotate.drug.response.outliers_(drug.data.long, drug.screen.cols, conc.col, response.col, 
                                                   ...
                                                   max_response = max.response, prefix = prefix)
}
