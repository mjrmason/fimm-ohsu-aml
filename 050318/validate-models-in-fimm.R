...
## drug.feature.syms[["Pelitinib"]] <- NA
## drug.feature.syms[["Neratinib"]] <- NA
...
drug.to.feature.map <- data.frame(drug.name = overlapping.drugs[, "FIMM_DRUG_NAME"], drug = overlapping.drugs[, drug.map.drug.id.cols[[data.set]]], features = overlapping.drugs$FIMM_DRUG_NAME, stringsAsFactors = FALSE)
