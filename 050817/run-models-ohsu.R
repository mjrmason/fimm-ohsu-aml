...
cat("This script in 050817/run-models.R performs LOOCV of z-scored IC50 vs z-scored expression\n")
...
correlate.expression.with.ic50 <- function(drug.df, drug.id.col, drugs.to.test, zscore.response = FALSE, zscore.expression = FALSE, 
  ...
  ret <- ddply(df, drug.id.col, .parallel = TRUE,
               .fun = function(df.drug) {
                 ...
                 drug.ret <- ldply(drug.targets, .parallel = FALSE,
                       .fun = function(gene) {
                         ...
                         ## Perform LOOCV
                         pvals <- rep(NA, length(gene.expr))
                         r2s <- rep(NA, length(gene.expr))
                         predicteds <- rep(NA, length(gene.expr))
                         actuals <- rep(NA, length(gene.expr))

                         for (test_i in 1:length(gene.expr)) {
                           ...
                           pvals[test_i] <- p
                           r2s[test_i] <- r2
                           predicteds[test_i] <- pred
                           actuals[test_i] <- actual
                         }
                         ...
                         r2s <- paste(pvals, collapse=",")
                         predicteds <- paste(predicteds, collapse=",")
                         actuals <- paste(actuals, collapse=",")
                         res <- c(gene, pvals, r2s, predicteds, actuals, num.resp.samples, num.unique.resp.samples, num.samples, num.unique.samples)
                         names(res) <- c("gene", "pvals", "r2s", "predicted", "actual", "num.resp", "num.uniq.resp", "num.samples", "num.uniq.samples")
                         res
                       })
                       ## To save some space, set the actual values to NA for all but the first row for a given drug since they are
                       ## the same across the drug (i.e., a function of drug, not gene)
                       if(nrow(drug.ret) > 1) {
                         drug.ret$actual[2:nrow(drug.ret)] <- NA
                       }
                       drug.ret
               })
  colnames(ret) <- c(drug.id.col, "gene", "pvals", "r2s", "predicted", "actual", "num.resp", "num.uniq.resp", "num.samples", "num.uniq.samples")
  ret
}
...
##common.drugs.tested <- common.drugs.tested[1:2]
##common.genes <- common.genes[1:20]
...
## Correlating gene expr with IC50 for drug FIMM133780
## Correlating gene expr with IC50 for drug FIMM133809
## Correlating gene expr with IC50 for drug FIMM133818
...
plot.expr.vs.ic50 <- function(df, drug, ensg.gene, expr.mat, xlab, ylab, drug.id.col = "DRUG_ID", patient.id.col = "PATIENT_ID", specimen.id.col = "SCREEN_ID", invert.response = TRUE) {
  ...
  resp <- resp[!is.infinite(resp$IC50) & !is.nan(resp$IC50) & !is.na(resp$IC50),]
  if(invert.response) { resp$IC50 <- - resp$IC50 }
  common.samples <- intersect(resp$id, colnames(expr.mat))
  flag <- resp$id %in% common.samples
  resp <- resp[flag,]
  tmp <- as.data.frame(table(resp$PATIENT_ID))
  ...
  ct <- cor.test(df$expr, df$resp)
  g <- ggplot(df, aes(x = expr, y = resp, weight=weights)) + 
    geom_point() + 
    xlab(xlab) + ylab(ylab) +
    stat_smooth(method = "lm", col = "red")  
  return(list(g = g, corr = ct$estimate, corr.pval = ct$p.value))
}
...
## We did not set the actual values for many of the rows, since they would be repeated across drugs.
## Fill those in now.
actual.df <- na.omit(ohsu.fimm.expr.corr[,c("ID_Drug", "actual.ohsu", "actual.fimm")])
if(any(duplicated(actual.df[,c("ID_Drug")]))) {
  warning("Was not expecting dup actuals")
}
ohsu.fimm.expr.corr <- ohsu.fimm.expr.corr[,!(grepl(colnames(ohsu.fimm.expr.corr), pattern="actual"))]
old.nrow <- nrow(ohsu.fimm.expr.corr)
ohsu.fimm.expr.corr <- merge(ohsu.fimm.expr.corr, actual.df)
if(old.nrow != nrow(ohsu.fimm.expr.corr)) {
  warning("Table size changed after merging in actuals")
}
...
## head(ohsu.fimm.expr.corr[,c("ID_Drug", "gene", "DRUG_NAME", "actual.ohsu", "predicted.ohsu")]), c("ID_Drug", "gene"),
indices <- (1:nrow(ohsu.fimm.expr.corr))[ohsu.fimm.expr.corr$DRUG_NAME == "Tamatinib"]
indices <- (1:nrow(ohsu.fimm.expr.corr))
tmp <- 
ldply(indices, 
      .parallel = TRUE,
      .fun = function(i) {
        id.drug <- ohsu.fimm.expr.corr$ID_Drug[i]
        gene <- ohsu.fimm.expr.corr$gene[i]
        drug.name <- ohsu.fimm.expr.corr$DRUG_NAME[i]
        actual.ohsu <- as.numeric(unlist(strsplit(ohsu.fimm.expr.corr$actual.ohsu[i], split=",[ ]*")))
        predicted.ohsu <- as.numeric(unlist(strsplit(ohsu.fimm.expr.corr$predicted.ohsu[i], split=",[ ]*")))
        ct.ohsu <- cor.test(actual.ohsu, predicted.ohsu)
        
        actual.fimm <- as.numeric(unlist(strsplit(ohsu.fimm.expr.corr$actual.fimm[i], split=",[ ]*")))
        predicted.fimm <- as.numeric(unlist(strsplit(ohsu.fimm.expr.corr$predicted.fimm[i], split=",[ ]*")))
        ct.fimm <- cor.test(actual.fimm, predicted.fimm)
        
        vec <- c(id.drug, gene, drug.name, ohsu.fimm.expr.corr$actual.ohsu[i], ohsu.fimm.expr.corr$predicted.ohsu[i], ct.ohsu$p.value, ct.ohsu$estimate, ohsu.fimm.expr.corr$actual.fimm[i], ohsu.fimm.expr.corr$predicted.fimm[i], ct.fimm$p.value, ct.fimm$estimate)
        names(vec) <- c("ID_Drug", "gene", "DRUG_NAME", "actual.ohsu", "predicted.ohsu", "corr.pval.ohsu", "corr.ohsu", "actual.fimm", "predicted.fimm", "corr.pval.fimm", "corr.fimm")
        vec
      })

save.image(".RData.tbl")

all.genes <- unique(tmp$gene)
ensg.to.sym <- ensg.to.symbols.mapping(all.genes)

old.nrow <- nrow(tmp)
ohsu.fimm.expr.corr.gene <- merge(tmp, unique(ensg.to.sym), by.x = "gene", by.y = "ENSG")

ohsu.fimm.expr.corr.gene$corr.ohsu <- as.numeric(ohsu.fimm.expr.corr.gene$corr.ohsu)
ohsu.fimm.expr.corr.gene$corr.fimm <- as.numeric(ohsu.fimm.expr.corr.gene$corr.fimm)
ohsu.fimm.expr.corr.gene$corr.pval.ohsu <- as.numeric(ohsu.fimm.expr.corr.gene$corr.pval.ohsu)
ohsu.fimm.expr.corr.gene$corr.pval.fimm <- as.numeric(ohsu.fimm.expr.corr.gene$corr.pval.fimm)

## Plot scatter of those that have significant correlations (of actual vs predicted) in both
tbl <- subset(ohsu.fimm.expr.corr.gene, (corr.pval.ohsu < 0.05) & (corr.pval.fimm < 0.05))

cat(paste0("chisq.test of positive predicted/actual correlation of OHSU sig (p < 0.05) vs FIMM sig events: ", chisq.test(tbl$corr.ohsu > 0, tbl$corr.fimm > 0)$p.value, "\n"))
### chisq.test of positive predicted/actual correlation of OHSU sig (p < 0.05) vs FIMM sig events: 0.00826295931710042
chisq.test(tbl$corr.ohsu > 0, tbl$corr.fimm > 0)$expected
chisq.test(tbl$corr.ohsu > 0, tbl$corr.fimm > 0)$observed

## Pvalue distribution of FIMM and OHSU (LOOCV)
## Visualize overlap (LOOCV)
## Do genomic

## Add correlation of gene expr vs IC50
## Of those that are significant in both, how many have same sig of expr/ic50 correlation?

flag <- (sign(tbl$corr.ohsu) > 0) & (sign(tbl$corr.fimm) > 0)
tbl <- tbl[flag, ]
for(i in 1:nrow(tbl)) {
  drug.id <- tbl$ID_Drug[i]
  ensg.gene <- tbl$gene[i]
  drug.name <- tbl$DRUG_NAME[i]
  gene.name <- tbl$SYMBOL[i]
  drug.col <- "DRUG_ID"
  pt.col <- "PATIENT_ID"
  spec.col <- "SCREEN_ID"
  xlab <- paste0(gene.name, " expression (log CPM zscore)")
  ylab <- paste0(drug.name, " drug response (- zscore log IC50)")
  l1 <- plot.expr.vs.ic50(fimm.fits.df, drug.id, ensg.gene, fimm.expr.data, xlab = xlab, ylab = ylab, drug.id.col = drug.col, patient.id.col = pt.col, specimen.id.col = spec.col)
  drug.col <- "ID_Drug"
  pt.col <- "patient_id"
  spec.col <- "SeqID"
  l2 <- plot.expr.vs.ic50(ohsu.fits.df, drug.id, ensg.gene, ohsu.expr.data, xlab = xlab, ylab = ylab, drug.id.col = drug.col, patient.id.col = pt.col, specimen.id.col = spec.col)
  if(sign(l1[["corr"]]) != sign(l2[["corr"]])) { next }
...
tbl <- subset(ohsu.fimm.expr.corr.gene, (corr.ohsu > 0.5) & (corr.fimm > 0.5))

## Look for correlation of related drugs.
## Group drugs by shared targets
...
drug.mech <- ohsu.fimm.drugs[!is.na(ohsu.fimm.drugs$Ensg.Targets),c("ID_Drug", "Mechanism/Targets", "Ensg.Targets", "Gene.Targets")]
colnames(drug.mech) <- c("ID_Drug", "Mechanism", "Ensg.Targets", "Gene.Targets")

plot.rank <- TRUE

qt <- 
  dlply(drug.mech, c("Mechanism"), 
        .fun = function(df) {
          mechanism <- df$Mechanism[1]
          if(nrow(df) > 1) {
            drug.ids <- df$ID_Drug
            ##print(drug.ids)
            ensg.gene.targets <- unlist(strsplit(df$Ensg.Targets[1], split=",[ ]*"))
            ##gene.targets <- unlist(strsplit(df$Gene.Targets[1], split=",[ ]*"))
            expr.gene.targets <- ensg.gene.targets[ensg.gene.targets %in% rownames(ohsu.expr.data)]
            gene.targets <- ensg.gene.targets
            sb <- ohsu.fits.df[ohsu.fits.df$ID_Drug %in% drug.ids, c("inhibitor", "lab_id", "replicant", "IC50.z")]
            sb <- ddply(sb, c("inhibitor", "lab_id"), .fun = function(df) median(df$IC50.z))
            colnames(sb) <- c("inhibitor", "lab_id", "IC50.z")
            sp <- spread(sb, key = lab_id, value = IC50.z)
            rownames(sp) <- sp$inhibitor
            cat(paste0("Mechanism: ", mechanism, " drugs: ", paste(rownames(sp), collapse=", "), " targets: ", paste(gene.targets, collapse=", "), " expr targets: ", paste(expr.gene.targets, collapse=", "), "\n"))
            sp <- sp[, !(colnames(sp) == "inhibitor")]
            ##sp <- t(na.omit(t(sp)))
            ##class(sp) <- "matrix"
            sp <- sp[, order(sp[1,])]
 ##           if(plot.rank) {
##              sp <- t(apply(sp, 1, rank))
##            }
            
            ##print(head(sp[,1:5]))
            ##print(dim(sp))
            pdf(make.names(paste0("ohsu-mechanism-", mechanism, ".pdf")))
            if(plot.rank) {
cat("plot.rank.cor\n")
              pairs(t(sp), lower.panel=panel.regression, upper.panel=panel.rank.cor, main = paste0("OHSU ", mechanism))
            } else {
              pairs(t(sp), lower.panel=panel.regression, upper.panel=panel.cor, main = paste0("OHSU ", mechanism))
...
dlply(drug.mech, c("Mechanism"), 
      .fun = function(df) {
        if(nrow(df) > 1) {
          ...
          ensg.gene.targets <- unlist(strsplit(df$Ensg.Targets[1], split=",[ ]*"))
          ##gene.targets <- unlist(strsplit(df$Gene.Targets[1], split=",[ ]*"))
          expr.gene.targets <- ensg.gene.targets[ensg.gene.targets %in% rownames(fimm.expr.data)]
          gene.targets <- ensg.gene.targets
          sb <- fimm.fits.df[fimm.fits.df$DRUG_ID %in% drug.ids, c("DRUG_NAME", "PATIENT_ID", "SCREEN_ID", "IC50.z")]
          sb <- ddply(sb, c("DRUG_NAME", "PATIENT_ID"), .fun = function(df) median(df$IC50.z))
          colnames(sb) <- c("DRUG_NAME", "PATIENT_ID", "IC50.z")
          sp <- spread(sb, key = PATIENT_ID, value = IC50.z)
          rownames(sp) <- sp$DRUG_NAME
          cat(paste0("Mechanism: ", mechanism, " drugs: ", paste(rownames(sp), collapse=", "), " targets: ", paste(gene.targets, collapse=", "), " expr targets: ", paste(expr.gene.targets, collapse=", "), "\n"))
          sp <- sp[, !(colnames(sp) == "DRUG_NAME")]
          ## sp <- t(na.omit(t(sp)))
          ## class(sp) <- "matrix"
          sp <- sp[, order(sp[1,])]
##          if(plot.rank) {
...
##          }
          ...
          pdf(make.names(paste0("fimm-mechanism-", mechanism, ".pdf")))
          if(plot.rank) {
            cat("panel.rank.con\n")
            pairs(t(sp), lower.panel=panel.regression, upper.panel=panel.rank.cor, main = paste0("FIMM ", mechanism))
          } else {
            pairs(t(sp), lower.panel=panel.regression, upper.panel=panel.cor, main = paste0("FIMM ", mechanism))
          }
          d <- dev.off()
        }
      })

correlate.response.of.overlappling.drug.targets <- 
  function(df, fits.df, drug.id.col = "ID_Drug", fit.drug.id.col = "ID_Drug", 
           drug.col = "inhibitor", replicate.col = c("replicant"), spread.key = "lab_id", data.set = "ohsu", plot.rank = TRUE) {
                                                            
  df <- df[!is.na(df$Ensg.Targets),]

  ## Find all possible drug targets
  all.ensg.gene.targets <- unique(unlist(strsplit(paste(df$Ensg.Targets, collapse=", "), split=",[ ]*")))
  all.gene.targets <- unique(unlist(strsplit(paste(df$Gene.Targets, collapse=", "), split=",[ ]*")))
  all.gene.targets <- sort(all.gene.targets)
  
  reported.drug.ids <- c()
  
  for(gene.target in all.gene.targets) {
    flag <- unlist(lapply(df$Gene.Targets, function(targets) grepl(pattern=gene.target, targets)))
    df.target <- df[flag,]
    drug.ids <- df.target[, drug.id.col]
    tmp <- paste(drug.ids, collapse=",")
    if(tmp %in% reported.drug.ids) { next }
    reported.drug.ids <- c(reported.drug.ids, tmp)
    sb <- fits.df[fits.df[, fit.drug.id.col] %in% drug.ids, c(drug.col, spread.key, replicate.col, "IC50.z")]
    sb <- ddply(sb, c(drug.col, spread.key), .fun = function(df) median(df$IC50.z))
    colnames(sb) <- c(drug.col, spread.key, "IC50.z")
    colnames(sb)[colnames(sb) == spread.key] <- "key"
    sp <- spread(sb, key = key, value = IC50.z)
    rownames(sp) <- sp[,drug.col]
    sp <- sp[, !(colnames(sp) == drug.col)]
    ## sp <- t(na.omit(t(sp)))
##    print(sp)
##    class(sp) <- "matrix"
    if(ncol(sp) < 5) { next }
...
##      sp <- t(apply(sp, 1, rank))
##    }
    pdf(make.names(paste0(data.set, "-target-", gene.target, ".pdf")))
    if(plot.rank == FALSE) {
      pairs(t(sp), lower.panel=panel.regression, upper.panel=panel.cor, main = paste0(data.set, "-", gene.target))
    } else {
      cat("Doing panel.rank.cor")
      pairs(t(sp), lower.panel=panel.regression, upper.panel=panel.rank.cor, main = paste0(data.set, "-", gene.target))
      ## pairs(t(sp), xlim=c(1, ncol(sp)), ylim=c(1, ncol(sp)), lower.panel=panel.rank.regression, upper.panel=panel.rank.cor, main = paste0(data.set, "-", gene.target))
    }
    d <- dev.off()
  }
}

## correlate.response.of.overlappling.drug.targets(drug.mech[drug.mech[,1] %in% c("FIMM003719","FIMM003725"),], ohsu.fits.df , drug.id.col = "ID_Drug", fit.drug.id.col = "ID_Drug", drug.col = "inhibitor", replicate.col = c("replicant"), spread.key = "lab_id", data.set = "ohsu")
correlate.response.of.overlappling.drug.targets(drug.mech, ohsu.fits.df , drug.id.col = "ID_Drug", fit.drug.id.col = "ID_Drug", drug.col = "inhibitor", replicate.col = c("replicant"), spread.key = "lab_id", data.set = "ohsu")

correlate.response.of.overlappling.drug.targets(drug.mech, fimm.fits.df , drug.id.col = "ID_Drug", fit.drug.id.col = "DRUG_ID", drug.col = "DRUG_NAME", replicate.col = c("SCREEN_ID"), spread.key = "PATIENT_ID", data.set = "fimm")


## Smooth plot
## smoothScatter(ohsu.fimm.expr.corr$corr.ohsu, ohsu.fimm.expr.corr$corr.fimm)

## Of those sig in both, how many are in same direction?

## Which correlations (of expr vs IC50) are in the same direction?
## Of those sig in both, how many are in same direction?
...
ohsu.fimm.expr.corr$corr.ohsu <- unlist(apply(ohsu.fimm.expr.corr[,c("actual.ohsu", "predicted.ohsu")], 1,
                                              function(row) {
                                                actual <- as.numeric(unlist(strsplit(row[1], split=",[ ]*")))
                                                predicted <- as.numeric(unlist(strsplit(row[2], split=",[ ]*")))
                                                cor(actual, predicted)
                                              }))
ohsu.fimm.expr.corr$corr.fimm <- unlist(apply(ohsu.fimm.expr.corr[,c("actual.fimm", "predicted.fimm")], 1,
                                              function(row) {
                                                actual <- as.numeric(unlist(strsplit(row[1], split=",[ ]*")))
                                                predicted <- as.numeric(unlist(strsplit(row[2], split=",[ ]*")))
                                                cor(actual, predicted)
