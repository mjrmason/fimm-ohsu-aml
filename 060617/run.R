...
compute.ll.4.auc <- function(slope, min.asymptote, max.asymptote, ic50, min.conc, max.conc, force.ic50.in.range = TRUE) {
  ...
  b <- unname(slope)
  c <- unname(min.asymptote)
  d <- unname(max.asymptote)
  e <- unname(ic50)
  ...
  auc <- NA
  ## auc.x1 is the auc we use for dss
  auc.x1 <- NA
  ## x1 can not be negative (since we take the log in fnc)
  calculate.dss <- TRUE
  if(!is.finite(x1) || (x1 < 0) || (x1 >= max.conc)) {
    ## If x1 is not finite, just integrate from min.conc--but then do not calculate DSS.
    calculate.dss <- FALSE
  } else {
    val <- integrate(fnc, x1, max.conc)
    if(val$message == "OK") { auc.x1 <- val$value }
  }
  if(!is.finite(auc.x1)) { calculate.dss <- FALSE }
  ...
  dss <- c(NA, NA, NA)
  names(dss) <- c("dss1", "dss2", "dss3")
  if(is.finite(x1)) {
    dss <- compute.dss(auc.x1, ic50, t = t, c.min = min.conc, c.max = max.conc, x1 = x1, x2 = max.conc, top.asymptote = max.asymptote, force.ic50.in.range = force.ic50.in.range)
  }
  ## NB: the auc we return is the full auc, not auc.x1 used to calculate dss.
  vec <- c(auc, dss)
...
fit.fct.to.ohsu <- function(data, fct = "LL.4", force.ic50.in.range=TRUE) {
...
##          fit
          ...
          ic50 <- 0
          auc <- c(NA, NA, NA, NA)
          names(auc) <- c("auc", "dss1", "dss2", "dss3")
          slope <- 0
          min.asymptote <- 0
          max.asymptote <- 0
          ...
          if(!is.null(fit) && (fit$fit$convergence || fit$convergence)) {
            ...
            ic50 <- coef(fit)[4]
            if(fct == "LL.4") {
              auc <- compute.ll.4.auc(slope, min.asymptote, max.asymptote, ic50, min.conc, max.conc, force.ic50.in.range = force.ic50.in.range)
            } else {
          ...
          names(vec) <- c("slope", "min.asymptote", "max.asymptote", "ic50", "gof", names(auc), "min.conc", "max.conc")
          vec
        })
  }

fit.fct.to.fimm <- function(data, fct = "LL.4", force.ic50.in.range=TRUE) {
...
cat("Fitting LL4\n")
ll4.fits.dss.fixed <- fit.fct.to.ohsu(ohsu.raw.drug.data, fct = "LL.4", force.ic50.in.range=TRUE)
cat("Done fitting LL4\n")

cat("Fitting LL4 no force\n")
ll4.fits.dss.no.force.fixed <- fit.fct.to.ohsu(ohsu.raw.drug.data, fct = "LL.4", force.ic50.in.range=FALSE)
cat("Done fitting LL4 no force\n")
save.image(".RData")
...
fimm.ll4.dss.fits <- fit.fct.to.fimm(fimm.raw.drug.data, fct = "LL.4", force.ic50.in.range=TRUE)
fimm.ll4.dss.fits.no.force <- fit.fct.to.fimm(fimm.raw.drug.data, fct = "LL.4", force.ic50.in.range=FALSE)
