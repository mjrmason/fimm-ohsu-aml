...
lst <- bootstrap.elastic.net.ohsu(ohsu.dss.t0, ohsu.expr, ohsu.rnaseq.sample.summary, drugs[1:10], response.col = "auc.l4", alphas = c(0, 0.25, 0.5, 0.75, 1), num.bootstraps = 5)
...
predicted.responses <- ldply(1:length(lst), .parallel = FALSE, 
                             .fun = function(boot.i) {
                                      boot.lst <- lst[[boot.i]]
                                      boot.map <- boot.lst$bootstrap.sample.map
                                      ret.boot <- ldply(boot.lst$fits, .parallel = FALSE,
                                                        .fun = function(fit) {
...
if(length(resps) <= 1) { return(c(alpha = fit$alphas[i], predicted.response = NA, predicted.samples = NA)) }
                                                                                             ...
                                                                                             m <- merge(response, boot.map, by.x = "sample", by.y = "bootstrap.sample")
                                                                                             resp.str <- paste0(m$response, collapse=",")
                                                                                             sample.str <- paste0(m$orig.sample, collapse=",")
...
if(length(resps) <= 1) { return(c(drug = fit$drug, actual.response = NA, actual.samples = NA)) }
                                                                 ...
                                                                 resp.str <- paste0(m$response, collapse=",")
                                                                 sample.str <- paste0(m$orig.sample, collapse=",")
                                                                 vec <- c(fit$drug, resp.str, sample.str)
                                                                 names(vec) <- c("drug", "actual.response", "actual.samples")
                                                                 vec
                                                               })
                                      ret.boot$bootstrap <- boot.i
                                      ret.boot
...
## TODO
d_ply(short.df[short.df$drug == short.df$drug[1],], .variables = c("alpha", "drug"), .parallel = FALSE,
      .fun = function(df) {
                df$response <- as.numeric(df$response)
                predicted.flag <- df$response.type == "predicted"
                g <- ggplot(data = df[predicted.flag,])
                g <- g + geom_boxplot(aes(x = sample, y = response), col = "black")
                g <- g + geom_point(data = df[!predicted.flag,], aes(x = sample, y = response), col = "blue")
...
das.g <- plot.predicted.vs.actual.response(short.df, target.alpha = target.alpha, target.drug = target.drug)
print(das.g)
d <- dev.off()
...
mydoc <- addSubtitle( mydoc , 'June 27, 2017')

mydoc <- addSlide( mydoc, slide.layout = "Title and Content" )
## title <- "FIMM (t = 0)"
## mydoc <- addTitle( mydoc, title )
mydoc <- addPlot( doc = mydoc, fun = plot, x = das.g)
...
writeDoc(mydoc, "062717-elastic-net-ohsu.pptx")
