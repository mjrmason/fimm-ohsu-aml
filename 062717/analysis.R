               ...
               .fun = function(drug) {
                        ...
                        res.drug <- llply(1:num.bootstraps, .parallel = FALSE,
                                          .fun = function(boot.i) {
                                                   ...
                                                   fit <- fit.elastic.net.ohsu_(drc.train, expr.train, drc.test, expr.test, alphas, train.alphas.in.parallel = FALSE)
                                                   fit[["drug"]] <- drug
                                                   fit[["boot"]] <- boot.i
                                                   fit
                                          })
...
## alpha = 0: ridge
fit.elastic.net.ohsu_ <- function(y.train, X.train, y.test, X.test, alphas, train.alphas.in.parallel = FALSE) {
