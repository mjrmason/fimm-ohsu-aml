...
## FIMM drug ids
all.common.drugs <- na.omit(ohsu.fimm.drugs$ID_Drug)
mek.inhibitors <- ohsu.fimm.drugs$ID_Drug.ohsu[ohsu.fimm.drugs$`Mechanism/Targets` == "MEK1/2 inhibitor"]
...
## lst <- bootstrap.elastic.net.fimm(fimm.dss.t0, fimm.expr, drugs, response.col = "auc.l4", alphas = c(0, 0.25, 0.5, 0.75, 1), num.bootstraps = 50)
lst <- bootstrap.elastic.net.fimm(fimm.dss.t0, fimm.expr, drugs, response.col = "auc.l4", alphas = c(0, 0.25, 0.5, 0.75, 1), num.bootstraps = 2)
## mek.drugs <- c("FIMM003708", "FIMM003751", "FIMM133840")
## lst <- bootstrap.elastic.net.fimm(fimm.dss.t0, fimm.expr, mek.drugs, response.col = "auc.l4", alphas = c(0, 0.25, 0.5, 0.75, 1), num.bootstraps = 2)
