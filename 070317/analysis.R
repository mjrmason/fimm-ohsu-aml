...
loocv.elastic.net.fimm <- function(drc.df, expr.df, drugs, response.col, alphas, seed = 1234, remove.outlier = FALSE) {
  ...
  res <- llply(drugs, .parallel = TRUE,
               .fun = function(drug) {
                        cat(paste0("Fitting elastic net to ", drug, "\n"))
                        ## Leave out each sample

                        sample.indices <- which(!is.na(drc.df[drug,]))	
                        res.drug <- llply(sample.indices, .parallel = FALSE,
                                          .fun = function(i) {
                                                   set.seed(i)

                                                   sample.indices <- which(!is.na(drc.df[drug,]))	
                                                   train.indices <- sample.indices[sample.indices != i]
                                                   test.indices <- c(i)
                                                   ...
                                                   if( (length(train.indices) == 0) && (length(test.indices) == 0) ) {
                                                     ...
                                                     drc.train <- drc.df[drug, train.indices]
                                                     expr.train <- expr.df[, train.indices]
                                                     drc.test <- drc.df[drug, test.indices, drop=F]
                                                     expr.test <- expr.df[, test.indices, drop=F]

                                                     fit <- fit.elastic.net_(drc.train, expr.train, drc.test, expr.test, alphas, train.alphas.in.parallel = FALSE, do.zscore = FALSE)
                                                   }
                                                   fit[["drug"]] <- drug
                                                   fit[["loo"]] <- i
                                                   fit
...
loocv.elastic.net.ohsu <- function(drc.df, expr.df, rnaseq.summary.df, drugs, response.col, alphas, seed = 1234) {
...
## alpha = 0: ridge
fit.elastic.net_ <- function(y.train, X.train, y.test, X.test, alphas, train.alphas.in.parallel = FALSE, do.zscore = TRUE) {
...
post.process.fits <- function(lst, prefix) {
                            ...
                            .fun = function(drug.i) {
                              ...
                              ret.drug <- ldply(drug.lst, .parallel = FALSE,
                                                .fun = function(fit) {
                                                  ...
                                                  vec <- c(fit$drug, val, resp.str, sample.str)
                                                  names(vec) <- c("drug", name, "actual.response", "actual.samples")
                                                  vec
                                                })
                              ret.drug
                            })
  ...
  cat("Subsetting\n")
print(actual.predicted.correlations.df)
  tmp <- subset(actual.predicted.correlations.df, alpha == 0)
print(tmp)
  tmp <- tmp[order(tmp$pearson.p,decreasing=FALSE),,drop=F]
  
print(tmp)
  cat("Starting individual plots\n")
  ...
  cat("Done with individual plots\n")
  mydoc <- pptx()
