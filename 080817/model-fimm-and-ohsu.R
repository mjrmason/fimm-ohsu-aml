...
## Labs has information such as FAB, white blood count, etc.
ohsu.labs.tbl <- openxlsx::read.xlsx(getFileLocation(obj), sheet=3)

targeted.genes <- c("TET2", "MUTYH", "STAG2", "ATM", "CUX1", "CIITA", "ZRSR2", "STK11", "EP300", "WISP3", "GNAS", "NTRK2", "EZH2", "NF1", "MEN1", "POT1", "SF1", "SUZ12", "PHF6", "CAD", "KDM6A", "BRCA2", "APC", "BCORL1", "PDGFRbeta", "RAD21", "PAX5", "RAD50", "TCL1A", "STAT3", "LDH", "CEBPA", "NPM", "FLT3-D835", "FLT3-ITD", "JAK2", "KIT", "MCV", "ALT", "AST", "NRAS", "KRAS", "IDH1", "IDH2", "FLT3-ITD (internal)", "WT1", "T-cell Receptor Gene", "NPM1 (internal)", "TP53", "UTSW Genetic Testing (Foundation Medicine)", "RUNX1", "ASXL1", "AXIN1", "CD36", "SRSF2", "IKZF1", "DNMT3A", "CSF3R", "SETBP1", "CBL", "PTPN11", "No labs collected", "BRAF", "MPL", "No Genetic Testing", "BCOR", "U2AF1", "GATA1", "JAK3", "NOTCH1", "FLT3", "NRBC", "CCND2", "MLL2", "SF3B1", "GATA2", "CCND3", "CHEK2", "CTCF", "CREBBP", "TYK2", "ETV6", "ROS1", "KMT2A", "IGH", "FLT3-V491L", "FOXP1")

labs <- ohsu.labs.tbl$lab_type

cat(paste0(paste(unique(labs[!grepl(pattern="GeneTrail", labs, ignore.case=TRUE) & !grepl(pattern="Sequen", labs) & !(labs %in% targeted.genes)]), collapse=","), "\n"))

## Followup (sheet 4) has information such as disease_stage (amongst others Relapse, New Diagnosis) and age
ohsu.followup.tbl <- openxlsx::read.xlsx(getFileLocation(obj), sheet=4)
ohsu.followup.tbl <- subset(ohsu.followup.tbl, lab_id %in% ohsu.diagnosis.tbl$lab_id)
## ohsu.followup.tbl$disease_stage

## Treatment (sheet 5) has information such as response (amongst others Refractory).  Note that this is response
## relative to a specific drug.
ohsu.treatment.tbl <- openxlsx::read.xlsx(getFileLocation(obj), sheet=5)
ohsu.treatment.tbl <- subset(ohsu.treatment.tbl, lab_id %in% ohsu.diagnosis.tbl$lab_id)

## foo <- (ddply(ohsu.treatment.tbl, "patient_id", .fun = function(df) { paste(sort(unique(df$response)), collapse=",") }))
...
fimm.metadata$screen.id <- rownames(fimm.metadata)
...
m <- merge(ohsu.dss.arg[, c("lab_id", response.col)], unique(ohsu.treatment.tbl[, c("lab_id", "response")]))
m <- na.omit(m)
m$response <- factor(m$response)
lmfit <- lm(dss2.l4 ~ response, data = m)
summary(lmfit)
g <- ggplot(m, aes(x = response, y = dss2.l4))
g <- g + geom_violin()
g <- g + ylim(c(-5,5))

m <- merge(ohsu.dss.arg[, c("lab_id", response.col)], unique(ohsu.followup.tbl[, c("lab_id", "disease_stage")]))
m <- na.omit(m)
m$disease_stage <- factor(m$disease_stage)
lmfit <- lm(dss2.l4 ~ disease_stage, data = m)
summary(lmfit)
g <- ggplot(m, aes(x = disease_stage, y = dss2.l4))
g <- g + geom_violin()
g <- g + ylim(c(-5,5))

m <- merge(fimm.dss.arg[, c("SCREEN_ID", response.col)], unique(fimm.metadata[, c("screen.id", "diseases.stage")]), by.x = "SCREEN_ID", by.y = "screen.id")
m <- na.omit(m)
m$diseases.stage <- factor(m$diseases.stage)
lmfit <- lm(m[,response.col] ~ m$diseases.stage)
summary(lmfit)
g <- ggplot(m, aes_string(x = "diseases.stage", y = response.col))
g <- g + geom_violin()
g <- g + ylim(c(-1,1))
