...
##print(head(newx[1,]))
                         ...
                         for(alpha in alphas) {
                           for(s in svalues) {
                             ...
                             ret.list[[length(ret.list)+1]] <- list(train.drug = train.drug, test.drug = test.drug, predicted = pred, actual = newy, fit = fit, alpha = alpha, s = s, model = "glmnet")
                           }
                         }
                         ...
                         ret.list[[length(ret.list)+1]] <- list(train.drug = train.drug, test.drug = test.drug, predicted = pred, actual = newy, fit = fit, alpha = NA, s = NA, model = "rf")
                         ...
                         ret.list[[length(ret.list)+1]] <- list(train.drug = train.drug, test.drug = test.drug, predicted = pred, actual = newy, fit = fit, alpha = NA, s = NA, model = "svm")
                         ret.list
                })
   ret
} 
...
print(head(newy))
                         prob <- FALSE
                         if( (nrow(x) == 0) || (nrow(newx) == 0) || (ncol(x) == 0) || (ncol(newx) == 0) ) { prob <- TRUE }
}
...
plot.correlation.vs.drug <- function(tbl, model = "rf") {
     ...
     ldply(ret,
             ...
             .fun = function(df) {
                ...
                if((class(df$predicted) != "logical") && !is.na(df$predicted)) {
                  ...
                  ret <- data.frame(cor = ct$estimate, pval = ct$p.value, train.drug = df$train.drug, test.drug = df$test.drug, alpha = df$alpha, s = df$s, model = df$model)
                } else {
                  ret <- data.frame(cor = NA, pval = NA, train.drug = df$train.drug, test.drug = df$test.drug, alpha = df$alpha, s = df$s, model = df$model)
                }
                ret
             }) })
