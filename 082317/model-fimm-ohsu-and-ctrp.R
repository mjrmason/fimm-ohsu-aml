...
cat("Train on OHSU train\n")
ret <- train.model(train.dss.arg = ohsu.dss.orig.train, train.expr.arg = ohsu.expr.filt[common.genes, ], train.genomic.arg = NULL, train.clinical.arg = NULL, 
...
training.res[["ohsu.train"]] <- ret

cat("Train on OHSU test\n")
ret <- train.model(train.dss.arg = ohsu.dss.orig.test, train.expr.arg = ohsu.expr.filt[common.genes, ], train.genomic.arg = NULL, train.clinical.arg = NULL, 
                            train.common.drugs = ohsu.common.drugs, train.drugs = ohsu.query.drugs, train.drug.col = ohsu.drug.col,
                            train.patient.col = ohsu.patient.col, train.response.col = train.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE)  
training.res[["ohsu.test"]] <- ret
...
## Test everything against FIMM and ohsu.test
training.sets <- c("ctrp.all", "ctrp.heme", "ctrp.aml", "ohsu.train")
test.sets <- c("ohsu.test", "fimm")
for(train.set in training.sets) {
  train.drug.col <- ohsu.drug.col
  if(grepl(train.set, pattern="ctrp")) {
    train.drug.col <- ctrp.drug.col
  }

  for(test.set in test.sets) {
    test.dss.arg <- ohsu.dss.orig.test
    test.expr.arg <- ohsu.expr.filt[common.genes, ]
    test.common.drugs <- ohsu.common.drugs
    test.drugs <- ohsu.query.drugs
    test.drug.col <- ohsu.drug.col
    test.patient.col <- ohsu.patient.col
    if(grepl(test.set, pattern="fimm")) {
      test.dss.arg <- fimm.dss.orig
      test.expr.arg <- fimm.expr.filt[common.genes, ]
      test.common.drugs <- fimm.common.drugs
      test.drugs <- fimm.query.drugs
      test.drug.col <- fimm.drug.col
      test.patient.col <- fimm.patient.col
    }
    cat(paste0("Train on ", train.set, "; test on ", test.set, "\n"))
    ret <- test.model(fits = training.res[[train.set]], drug.name.tbl = drug.name.tbl, train.drug.col = train.drug.col, 
                      test.dss.arg = test.dss.arg, test.expr.arg = test.expr.arg, test.genomic.arg = NULL, test.clinical.arg = NULL, 
                      test.common.drugs = test.common.drugs, test.drugs = test.drugs, test.drug.col = test.drug.col,
                      test.patient.col = test.patient.col, test.response.col = test.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE)  
    test.res[[paste0(train.set, ".vs.", test.set)]] <- ret
  }
}
...
res.summary <- list()
for(test.set in test.sets) {
  all.res <- llply(training.sets,
                   .fun = function(train.set) {
                            cmp <- paste0(train.set, ".vs.", test.set)
                            cat(paste0("Extracting results for ", cmp, "\n"))
                            res <- extract.results(test.res[[cmp]])
                            res$train.set <- train.set
                            train.info <- extract.training.info(training.res[[train.set]])
                            res <- merge(res, train.info, by = c("train.drug", "alpha", "model"))
                            res
                          })
  all.res <- do.call("rbind", all.res)
  res.summary[[test.set]] <- all.res
}

num.samples <- list()
for(test.set in test.sets) {
  tmp <- res.summary[[test.set]]
  tmp <- unique(tmp[, c("test.drug", "n.train")])
  res <- ddply(tmp, .variables = c("test.drug"),
               .fun = function(df) {
                        min(df$n.train)
               })
  colnames(res) <- c("test.drug", "num.samples")
##  rownames(res) <- res$test.drug
##  res <- res[, c("num.samples"), drop=F]
  merge.col <- "master_cpd_id"
  if(test.set == "fimm") { 
    merge.col <- "DRUG_ID"
  } else if(test.set == "ohsu.test") {
    merge.col <- "inhibitor"
  }
  res <- merge(drug.name.tbl, res, by.y = "test.drug", by.x = merge.col)
  res <- res[order(res$inhibitor),]
  res <- res[, c("inhibitor", "DRUG_ID", "DRUG_NAME", "Mechanism", "master_cpd_id", "num.samples")]
  num.samples[[test.set]] <- res
}

## Number of samples in the training sets are independent of the test set,
## i.e., num.samples[[i]] == num.samples[[j]]   for all i,j
num.samples.per.drug <- num.samples[[test.sets[1]]]

num.iters <- 50

training.downsample.res <- list()

cat("Train on OHSU train with downsampling\n")
ret <- train.model.with.downsampling(train.dss.arg = ohsu.dss.orig.train, train.expr.arg = ohsu.expr.filt[common.genes, ], train.genomic.arg = NULL, train.clinical.arg = NULL, 
                                     train.common.drugs = ohsu.common.drugs, train.drugs = ohsu.query.drugs, train.drug.col = ohsu.drug.col,
                                     train.patient.col = ohsu.patient.col, train.response.col = train.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE,
                                     num.samples.per.drug = num.samples.per.drug, num.iterations = num.iters, with.replacement = FALSE)  
training.downsample.res[["ohsu.train"]] <- ret

if(FALSE) {
## Can not train these sets because we have not used these data sets to limit the number of samples per drug
## e.g., the FIMM data set is smaller and we will not have enough patients in that data set to sample the number
## of samples specified in num.samples.per.drug _with_ replacement.
cat("Train on OHSU test with downsampling\n")
ret <- train.model.with.downsampling(train.dss.arg = ohsu.dss.orig.test, train.expr.arg = ohsu.expr.filt[common.genes, ], train.genomic.arg = NULL, train.clinical.arg = NULL, 
                                     train.common.drugs = ohsu.common.drugs, train.drugs = ohsu.query.drugs, train.drug.col = ohsu.drug.col,
                                     train.patient.col = ohsu.patient.col, train.response.col = train.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE,
                                     num.samples.per.drug = num.samples.per.drug, num.iterations = num.iters, with.replacement = FALSE)  
training.downsample.res[["ohsu.test"]] <- ret

cat("Train on fimm with downsampling\n")
ret <- train.model.with.downsampling(train.dss.arg = fimm.dss.orig, train.expr.arg = fimm.expr.filt[common.genes, ], train.genomic.arg = NULL, train.clinical.arg = NULL, 
                                     train.common.drugs = fimm.common.drugs, train.drugs = fimm.query.drugs, train.drug.col = fimm.drug.col,
                                     train.patient.col = fimm.patient.col, train.response.col = train.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE,
                                     num.samples.per.drug = num.samples.per.drug, num.iterations = num.iters, with.replacement = FALSE)   
training.downsample.res[["fimm"]] <- ret
}

cat("Train on CTRP (All) with downsampling\n")
ret <- train.model.with.downsampling(train.dss.arg = ctrp.dss.orig, train.expr.arg = ctrp.expr.filt[common.genes, ], train.genomic.arg = NULL, train.clinical.arg = NULL, 
                                     train.common.drugs = ctrp.common.drugs, train.drugs = ctrp.query.drugs, train.drug.col = ctrp.drug.col,
                                     train.patient.col = ctrp.patient.col, train.response.col = train.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE,
                                     num.samples.per.drug = num.samples.per.drug, num.iterations = num.iters, with.replacement = FALSE)  
training.downsample.res[["ctrp.all"]] <- ret


cat("Train on CTRP (Heme) with downsampling\n")
ret <- train.model.with.downsampling(train.dss.arg = ctrp.dss.orig, train.expr.arg = ctrp.expr.filt[common.genes, heme.expr.cols], train.genomic.arg = NULL, train.clinical.arg = NULL, 
                                     train.common.drugs = ctrp.common.drugs, train.drugs = ctrp.query.drugs, train.drug.col = ctrp.drug.col,
                                     train.patient.col = ctrp.patient.col, train.response.col = train.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE,
                                     num.samples.per.drug = num.samples.per.drug, num.iterations = num.iters, with.replacement = FALSE)  
training.downsample.res[["ctrp.heme"]] <- ret

cat("Train on CTRP (AML) with downsampling\n")
ret <- train.model.with.downsampling(train.dss.arg = ctrp.dss.orig, train.expr.arg = ctrp.expr.filt[common.genes, aml.expr.cols], train.genomic.arg = NULL, train.clinical.arg = NULL, 
                                     train.common.drugs = ctrp.common.drugs, train.drugs = ctrp.query.drugs, train.drug.col = ctrp.drug.col,
                                     train.patient.col = ctrp.patient.col, train.response.col = train.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE,
                                     num.samples.per.drug = num.samples.per.drug, num.iterations = num.iters, with.replacement = FALSE)  
training.downsample.res[["ctrp.aml"]] <- ret

test.downsampled.models <- TRUE
if(test.downsampled.models) {

  tsets <- training.sets
  ## tsets <- c("ohsu.train", "ctrp.all")
  test.downsampled.res <- list()
  for(train.set in tsets) {
    train.drug.col <- ohsu.drug.col
    if(grepl(train.set, pattern="ctrp")) {
      train.drug.col <- ctrp.drug.col
    }
    ...
    for(test.set in test.sets) {
      ...
      cat(paste0("Train on ", train.set, "; test on ", test.set, "\n"))
      ret <- test.model(fits = training.downsample.res[[train.set]], drug.name.tbl = drug.name.tbl, train.drug.col = train.drug.col, 
                        test.dss.arg = test.dss.arg, test.expr.arg = test.expr.arg, test.genomic.arg = NULL, test.clinical.arg = NULL, 
                        test.common.drugs = test.common.drugs, test.drugs = test.drugs, test.drug.col = test.drug.col,
                        test.patient.col = test.patient.col, test.response.col = test.response.col, seed = 1234, use.rf = FALSE, use.svm = FALSE, flatten.downsampled.fits = TRUE)  
      test.downsampled.res[[paste0(train.set, ".vs.", test.set)]] <- ret
    }
  }

  res.downsampled.summary <- list()
  for(test.set in test.sets) {
    all.res <- llply(tsets,
                     .fun = function(train.set) {
                              cmp <- paste0(train.set, ".vs.", test.set)
                              cat(paste0("Extracting results for ", cmp, "\n"))
                              res <- extract.results(test.downsampled.res[[cmp]])
                              res$train.set <- train.set
                              train.info <- extract.training.info(training.downsample.res[[train.set]], flatten.downsampled.fits = TRUE)
                              train.info <- unique(train.info[, c("train.drug", "alpha", "model", "n.train")])
                              res <- merge(res, train.info, by = c("train.drug", "alpha", "model"))
                              res
                            })
    all.res <- do.call("rbind", all.res)
    ...
    all.res <- merge(drug.name.tbl, all.res, by.x = test.drug.col, by.y = "test.drug") 
    res.downsampled.summary[[test.set]] <- all.res
  }

  trends <- plot.all.downsampled.trends(res.downsampled.summary)

  for(test.set in test.sets) {
    trend.tbl <- trends[[test.set]]
    trend.tbl <- unique(trend.tbl[, c("inhibitor", "trend", "Class", "Mechanism")])
    cat(paste0(test.set, ": trend vs ", "class"))
    tbl <- table(trend.tbl$Class, trend.tbl$trend)
##    tbl <- tbl[tbl[, "mono"] > 0, , drop=F]
    print(tbl)
    cat("\n")

    cat(paste0(test.set, ": trend vs ", "mechanism"))
    tbl <- table(trend.tbl$Mechanism, trend.tbl$trend)
    tbl <- tbl[tbl[, "mono"] > 0, , drop=F]
    print(tbl)
    cat("\n")

    trend.tbl <- trend.tbl[!is.na(trend.tbl$trend),]
    trend.tbl <- trend.tbl[order(trend.tbl$inhibitor),]
    cat(paste0(test.set, " mono drugs: ", paste(trend.tbl$inhibitor[trend.tbl$trend == "mono"], collapse=", "), "\n"))
    cat("\n")
  }
}


## plot.all.trends(res.summary)
...
for(set.name in gsea.gene.sets$geneset.names) {
    ...
    if(length(ensg.set) > 0) {
      hallmark.gene.sets[[set.name]] <- ensg.set$ensg
    }
}
...
file <- getFileLocation(obj)
gsea.gene.sets <- GSA.read.gmt(file)
biocarta.gene.sets <- list()
for(set.name in gsea.gene.sets$geneset.names) {
    symbol.set <- gsea.gene.sets$genesets[[which(gsea.gene.sets$geneset.names == set.name)]]
    cat(paste0("Translating symbols to ensembl IDs for gene set ", set.name, "\n"))
    ensg.set <- na.omit(symbols.to.ensg.mapping(symbol.set))
    if(length(ensg.set) > 0) {
      biocarta.gene.sets[[set.name]] <- ensg.set$ensg
    }
}
...
for(set.name in gsea.gene.sets$geneset.names) {
    ...
    ensg.set <- na.omit(symbols.to.ensg.mapping(symbol.set))
    if(length(ensg.set) > 0) {
      kegg.gene.sets[[set.name]] <- ensg.set$ensg
    }
}
...
file <- getFileLocation(obj)
gsea.gene.sets <- GSA.read.gmt(file)
reactome.gene.sets <- list()
for(set.name in gsea.gene.sets$geneset.names) {
    symbol.set <- gsea.gene.sets$genesets[[which(gsea.gene.sets$geneset.names == set.name)]]
    cat(paste0("Translating symbols to ensembl IDs for gene set ", set.name, "\n"))
    ensg.set <- na.omit(symbols.to.ensg.mapping(symbol.set))
    if(length(ensg.set) > 0) {
      reactome.gene.sets[[set.name]] <- ensg.set$ensg
    }
}
...
res <- train.and.test.crossproduct(gene.sets = gene.sets, drug.name.tbl = drug.name.tbl, train.set.names = train.set.names, train.dss.args = train.dss.args, 
                            ...
                            .fun = function(test.set) {
                                     ldply(names(gene.sets),
                                           .fun = function(gene.set) {
                                                    tmp <- extract.results(res[[train.set]][[test.set]][[gene.set]])
                                                    tmp$train.set <- train.set
                                                    tmp$test.set <- test.set
                                                    tmp$gene.set <- gene.set
                                                    tmp                         
                                                  }) 
                                   }) 
                    })

sub <- subset(tbl, metric == "pearson")
g <- ggplot(data = sub, aes(x = gene.set, y = val))
g <- g + facet_grid(train.set ~ test.set)
g <- g + geom_point()
g <- g + ggtitle("AUC (Pearson)")
print(g)

pdf("baz.pdf")
do.call("grid.arrange", list(g, g, g, g))
dev.off()
