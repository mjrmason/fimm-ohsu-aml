...
## Read in the CTRP expression data (this is tpm)
synId <- "syn5616092"
obj <- synGet(synId, downloadFile = TRUE)
file <- getFileLocation(obj)
...
drug.name.tbl <- merge(drug.name.tbl, drug.target.tbl[, c("OHSU_DRUG_NAME", "Gene.Targets")], by.x = "inhibitor", by.y = "OHSU_DRUG_NAME")
...
ret <- bootstrap.model(train.dss.arg = ctrp.dss.orig, train.expr.arg = ctrp.expr.filt[common.genes, aml.expr.cols], train.common.drugs = ctrp.common.drugs, train.drugs = ctrp.query.drugs, 
...
ret <- bootstrap.model(train.dss.arg = ctrp.dss.orig, train.expr.arg = ctrp.expr.filt[common.genes, ], train.common.drugs = ctrp.common.drugs, train.drugs = ctrp.query.drugs, 
                       train.drug.col = ctrp.drug.col, train.patient.col = ctrp.patient.col, train.response.col = train.response.col, num.bootstraps = 100)
