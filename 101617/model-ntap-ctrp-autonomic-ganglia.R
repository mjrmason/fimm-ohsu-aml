...
data.set.dss.fit.synIds <- c("syn11267221", "syn11267372")
names(data.set.dss.fit.synIds) <- data.sets
data.set.dss.fit.files <- c("ctrp-nc-ag-dss-t0.tsv", "ntap-nc-ag-dss-t0.tsv")

## Batch-corrected log2 cpm expression files
data.set.expr.synIds <- c("syn11268911", "syn11268912")
names(data.set.expr.synIds) <- data.sets
data.set.expr.files <- c("ctrp-expr-nc-ag-combat.tsv", "ntap-expr-nc-ag-combat.tsv")
...
## Here "nc" refers to "ntap-ctrp-ag"; ag = autonomic ganglia
file.prefix <- "nc-ag"
