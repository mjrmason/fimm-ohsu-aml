...
data.set.dss.fit.synIds <- c("syn11267085", "syn11267086")
names(data.set.dss.fit.synIds) <- data.sets
data.set.dss.fit.files <- c("ctrp-nc-skin-dss-t0.tsv", "ntap-nc-skin-dss-t0.tsv")

## Batch-corrected log2 cpm expression files
data.set.expr.synIds <- c("syn11268018", "syn11268019")
names(data.set.expr.synIds) <- data.sets
data.set.expr.files <- c("ctrp-expr-nc-skin-combat.tsv", "ntap-expr-nc-skin-combat.tsv")
...
## Here "nc" refers to "ntap-ctrp-skin"
file.prefix <- "nc-skin"
