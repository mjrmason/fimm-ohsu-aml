...
drug.mapping.synId <- "syn11297396" ## ctrp-sanger-drug-map.tsv
drug.map.drug.id.cols <- c("master_cpd_id", "Drug.ID")

## These fit files are assumed to be for AUCs/DSS values calculated across shared concentration ranges across data sets.
data.set.dss.fit.synIds <- c("syn11297525", "syn11297571")
names(data.set.dss.fit.synIds) <- data.sets
data.set.dss.fit.files <- c("ctrp-cs-dss-t0.tsv", "sanger-cs-dss-t0.tsv")

## Batch-corrected log2 cpm (or rma) expression files
data.set.expr.synIds <- c("foo", "bar")
names(data.set.expr.synIds) <- data.sets
data.set.expr.files <- c("ctrp-expr-cs-combat.tsv", "sanger-expr-cs-combat.tsv")
...
## Here, we will store all in FIMM_BEAT AML Collaboration/Files/Analyses/ctrp-sanger (syn11297344)
fit.folder.synIds <- list("ctrp" = "syn11297344", "sanger" = "syn11297344")
...
patient.id.cols <- list("ctrp" = "ccl_name", "sanger" = "COSMIC_ID")
patient.subsets <- list("ctrp" = ccle.subset$ccl_name, "sanger" = sanger.subset$COSMIC.identifier)
patient.subsets <- list("ctrp" = NULL, "sanger" = NULL)
...
##  ft <- subset(ft, gof.ll4.ctrp > 0.8 & gof.ll4.sanger > 0.8)

  cat(paste0(length(unique(ft$cpd_name)), " compounds compared between Sanger and CTRP\n"))
  cat(paste0(length(unique(ft$COSMIC_ID)), " cell lines compared between Sanger and CTRP\n"))
  cat(paste0(nrow(ft), " fits compared between Sanger and CTRP\n"))
  pdf("ctrp-vs-sanger-auc.pdf")
  smooth.plot.r2(ft$dss.auc.ll4.ctrp, ft$dss.auc.ll4.sanger, xlab = "CTRP AUC", ylab = "Sanger AUC")
  dev.off()
  smooth.plot.r2(ft$e.ll4.ctrp, ft$e.ll4.sanger, xlab = "CTRP IC50", ylab = "Sanger IC50")

  ## Calculate correlations across patients
  corr.pt.df <- ddply(ft, .variables = c("cpd_name"),
                   .fun = function(df) {
                            ct <- cor.test(df$dss.auc.ll4.ctrp, df$dss.auc.ll4.sanger, method="spearman")
                            cor <- ct$estimate
                            pval <- ct$p.value
                            vec <- c(as.numeric(cor), as.numeric(pval))
                            names(vec) <- c("spearman.cor", "pval")
                            vec
                   })
  corr.pt.df$nlpv <- -log10(corr.pt.df$pval)
  corr.pt.df$pvalue <- rep(">= 0.05", nrow(corr.pt.df))
  corr.pt.df$pvalue[!is.na(corr.pt.df$pval) & (corr.pt.df$pval < 0.05)] <- "< 0.05"
  g1 <- ggplot(data = corr.pt.df, aes(x = spearman.cor, y = nlpv, colour = pvalue))
  g1 <- g1 + geom_point()
  g1 <- g1 + xlab(paste0("Rank Correlation (n = ", length(unique(ft$cpd_name)), "; one per drug)"))
  g1 <- g1 + ylab("- Log10 P-value")
  g1 <- g1 + geom_hline(yintercept = -log10(0.05), linetype = "dashed")

  ## Calculate correlations across drugs
  corr.drug.df <- ddply(ft, .variables = c("ccl_name"),
                   .fun = function(df) {
                            if(nrow(df) < 3) { return(c(NA, NA)) }
                            ct <- cor.test(df$dss.auc.ll4.ctrp, df$dss.auc.ll4.sanger, method="spearman")
                            cor <- ct$estimate
                            pval <- ct$p.value
                            vec <- c(as.numeric(cor), as.numeric(pval))
  ...
  corr.drug.df$nlpv <- -log10(corr.drug.df$pval)
  corr.drug.df$pvalue <- rep(">= 0.05", nrow(corr.drug.df))
  corr.drug.df$pvalue[!is.na(corr.drug.df$pval) & (corr.drug.df$pval < 0.05)] <- "< 0.05"
  g2 <- ggplot(data = corr.drug.df, aes(x = spearman.cor, y = nlpv, colour = pvalue))
  g2 <- g2 + geom_point()
  g2 <- g2 + xlab(paste0("Rank Correlation (n = ", length(unique(ft$ccl_name)), "; one per cell line)"))
  g2 <- g2 + ylab("- Log10 P-value")
  g2 <- g2 + geom_hline(yintercept = -log10(0.05), linetype = "dashed")

  pdf("sanger-vs-ctrp-volanco.pdf")
  grid.arrange(g1, g2)
  dev.off()

  g1 <- ggplot(data = corr.pt.df, aes(x = pvalue, y = spearman.cor, colour = pvalue))
  g1 <- g1 + geom_violin()
  g1 <- g1 + geom_beeswarm()
  g1 <- g1 + ggtitle(paste0("Rank Correlation (n = ", length(unique(ft$cpd_name)), "; one per drug)"))

  g2 <- ggplot(data = corr.drug.df, aes(x = pvalue, y = spearman.cor, colour = pvalue))
  g2 <- g2 + geom_violin()
  g2 <- g2 + geom_beeswarm()
  g2 <- g2 + ggtitle(paste0("Rank Correlation (n = ", length(unique(ft$ccl_name)), "; one per cell line)"))

  pdf("sanger-vs-ctrp-violin.pdf")
  grid.arrange(g1, g2)
  dev.off()
  ...
  pdf("sanger-vs-ctrp-boxplot.pdf")
  grid.arrange(g1, g2)
  dev.off()
...
smooth.plot.r2 <- function(x, y, labels = NULL, ...) {
  df <- data.frame(x = x, y = y)
  x.min <- min(df$x, na.rm=TRUE)
  x.max <- max(df$x, na.rm=TRUE)
  y.min <- min(df$y, na.rm=TRUE)
  y.max <- max(df$y, na.rm=TRUE)
  xlim <- c(min(x.min, y.min), max(x.max, y.max))
  ylim <- xlim
  smoothScatter(x, y, xlim = xlim, ylim = ylim, ...)
  reg <- lm(y ~ x, data = df)
  abline(reg)

  text(x = x.min + 0.5 * (x.max - x.min), y = y.min + 1 * (y.max - y.min), labels = paste0("r2 = ", format(summary(reg)$r.squared, digits=3)))
}
