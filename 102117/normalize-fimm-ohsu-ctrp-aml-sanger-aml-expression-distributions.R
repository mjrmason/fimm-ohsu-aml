...
data.set.expr.synIds <- c("syn10083723", "syn11261484", "syn11288436", "syn11288849")
names(data.set.expr.synIds) <- data.sets
data.set.expr.files <- c("ohsu.expr.tsv", "ctrpv2-log-cpm-gene-expr.tsv", "sanger-rma-gene-expr.tsv", "fimm-log-cpm-gene-expr.tsv")
...
## FIMM_BEAT AML Collaboration/Files/CTRPv2 Data (syn10288724)
## FIMM_BEAT AML Collaboration/Files/Sanger Data (syn11287866)
## FIMM_BEAT AML Collaboration/Files/Fimm Data/Processed Data (syn8270577)
## FIMM_BEAT AML Collaboration/Files/BEAT AML Data/Processed Data (syn10083332)
## FIMM_BEAT AML Collaboration/Files/Analyses/ctrp-sanger-fimm-ohsu (syn11288886)
output.folder.synIds <- list("ohsu" = "syn11288886", "ctrp" = "syn11288886", "sanger" = "syn11288886", "fimm" = "syn11288886")
...
x.sort <- sort(all.pca$x[,1], decreasing=FALSE)
outliers <- names(x.sort)[1:2]
if(!(all(outliers %in% c("14-00800", "20-00062")))) {
  stop(paste0("Was expected outliers: ", paste(c("14-00800", "20-00062"), collapse=","), " but got: ", paste(outliers, collapse=","), "\n"))
}
...
## We should exclude these -- do that elsewhere.
...
png(file)
## par(mfrow=c(2,1))
## g1 <- varplot(all.pca, cols=as.numeric(as.factor(batch.data.sets)), Main = "Uncorrected")
cols <- batch.data.sets
cols <- as.factor(cols)
...
##legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA, legend = unique(as.factor(batch.data.sets)), fill = unique(as.factor(batch.data.sets)), ncol=2)
## g2 <- varplot(all.combat.pca, cols=as.numeric(as.factor(batch.data.sets)), Main = paste0("Study Corrected (", paste(data.sets, collapse=","), ")"))
g2 <- varplot(all.combat.pca, cols=cols, Main = paste0("Study Corrected (", paste(data.sets, collapse=","), ")"))
## legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA, legend = unique(as.factor(batch.data.sets)), fill = unique(as.factor(batch.data.sets)))
##legend("top", legend = unique(as.factor(batch.data.sets)), fill = unique(as.factor(batch.data.sets)), ncol=2)
g2 <- g2 + coord_fixed()
grid.arrange(g1,g2)
dev.off()
