...
data.set.drug.fit.synIds <- c("syn10083494", "syn10307313", "syn11289087", "syn10083488")
names(data.set.drug.fit.synIds) <- data.sets
data.set.drug.fit.files <- c("ohsu.dss.t0.tsv", "ctrp.l4.and.ll4.fits.non.censored.pts.non.censored.curves.tsv", "sanger.l4.and.ll4.fits.tsv", "fimm.dss.t0.tsv")

data.set.drug.id.cols <- c("inhibitor", "master_cpd_id", "DRUG_ID", "DRUG_ID")
screen.id.cols <- list("ohsu" = c("patient_id", "inhibitor", "lab_id", "replicant", "time_of_read"),
                       "ctrp" = c("experiment_id", "master_cpd_id", "master_ccl_id"), 
...
## FIMM_BEAT AML Collaboration/Files/Analyses/ctrp-sanger-fimm-ohsu/expr-outlier-1 (syn11307535)
fit.folder.synIds <- list("ohsu" = "syn11307535", "ctrp" = "syn11307535", "sanger" = "syn11307535", "fimm" = "syn11307535")
## A string that will be included in the fit file name.
## Here "foc-aml-s-aml" = "fimm-ohsu-ctrp-aml-s-aml"
fit.file.prefix <- "foc-aml-s-aml-outlier1"
...
patient.id.cols <- list("ohsu" = "patient_id", "ctrp" = "master_ccl_id", "sanger" = "COSMIC_ID", "fimm" = "PATIENT_ID")
patient.subsets <- list("ohsu" = NULL, "ctrp" = ccle.subset$master_ccl_id, "sanger" = sanger.subset$COSMIC.identifier, "fimm" = NULL)

expr.patient.id.cols <- list("ohsu" = "SeqID", "ctrp" = "ccl_name", "sanger" = "COSMIC_ID", "fimm" = "SCREEN_ID")
