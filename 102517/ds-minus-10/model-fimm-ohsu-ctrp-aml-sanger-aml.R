...
offset <- - 10
fit.file.prefix <- paste0("foc-aml-s-aml-outlier-ds-offset-", offset)
...
num.train.samples.per.drug[, min.cnt.col] <- unlist(apply(num.train.samples.per.drug[, cnt.cols], 1, function(row) min(row) + offset))

cnt.cols <- grepl(colnames(num.test.samples.per.drug), pattern="test.num.samples")
num.test.samples.per.drug[, min.cnt.col] <- unlist(apply(num.test.samples.per.drug[, cnt.cols], 1, function(row) min(row) + offset))
