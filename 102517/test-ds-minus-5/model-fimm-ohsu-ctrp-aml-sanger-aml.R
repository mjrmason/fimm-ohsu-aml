...
offset <- - 5
fit.file.prefix <- paste0("foc-aml-s-aml-outlier-ds-test-offset-", offset)
...
num.train.samples.per.drug[, min.cnt.col] <- unlist(apply(num.train.samples.per.drug[, cnt.cols, drop=FALSE], 1, function(row) max(2,min(row) + offset)))

cnt.cols <- grepl(colnames(num.test.samples.per.drug), pattern="test.num.samples")
num.test.samples.per.drug[, min.cnt.col] <- unlist(apply(num.test.samples.per.drug[, cnt.cols, drop=FALSE], 1, function(row) max(2,min(row) + offset)))
