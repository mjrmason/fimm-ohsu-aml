...
data.sets.to.analyze <- c("ctrp", "fimm")
...
offset <- 0
fit.file.prefix <- paste0("foc-aml-s-aml-outlier-ds-test")
...
num.train.samples.per.drug[, min.cnt.col] <- unlist(apply(num.train.samples.per.drug[, cnt.cols, drop=FALSE], 1, function(row) min(row) + offset))

cnt.cols <- grepl(colnames(num.test.samples.per.drug), pattern="test.num.samples")
num.test.samples.per.drug[, min.cnt.col] <- unlist(apply(num.test.samples.per.drug[, cnt.cols, drop=FALSE], 1, function(row) min(row) + offset))
...
extract.drug <- function(lst, drug) {
  for(i in 1:length(lst)) {
    if(lst[[i]][[1]]$train.drug == drug) { return(lst[[i]]) }
  }
}
...
## Summarize across downsamplings (take median)
...
if(FALSE) {
load("../test-ds/.Rdata.focs")
tbl.full.test.dc <- tbl.full
load("../test-ds-minus-5/.Rdata.focs")
tbl.full.test.dc.minus.5 <- tbl.full
load("../test-ds-minus-10/.Rdata.focs")
tbl.full.test.dc.minus.10 <- tbl.full

load("../test-ds/.Rdata.focs")
tbl.test.dc <- tbl
load("../test-ds-minus-5/.Rdata.focs")
tbl.test.dc.minus.5 <- tbl
load("../test-ds-minus-10/.Rdata.focs")
tbl.test.dc.minus.10 <- tbl

tbl.merge <- merge(tbl.test.dc, tbl.test.dc.minus.5, suffixes = c(".test.dc", ".test.dc.5"), 
                   by = c("train.drug", "test.drug", "alpha", "s", "model", "metric", "train.set", "test.set", "gene.set", "response"))
tbl.merge <- merge(tbl.merge, tbl.test.dc.minus.10, suffixes = c("", ".test.dc.10"),
                   by = c("train.drug", "test.drug", "alpha", "s", "model", "metric", "train.set", "test.set", "gene.set", "response"))
tbl.merge <- subset(tbl.merge, model == "lasso" & metric == "spearman")
flag <- ( tbl.merge$val > tbl.merge$val.test.dc.5 ) & ( tbl.merge$val.test.dc.5 > tbl.merge$val.test.dc )

tbl.drug.test.dc.minus.10 <- extract.drug(res.auc.test.dc.minus.10[["all.comparisons"]][["ctrp"]][["fimm"]][["gene"]], drug)
tbl.drug.test.dc.minus.5 <- extract.drug(res.auc.test.dc.minus.5[["all.comparisons"]][["ctrp"]][["fimm"]][["gene"]], drug)
tbl.drug.test.dc <- extract.drug(res.auc.test.dc[["all.comparisons"]][["ctrp"]][["fimm"]][["gene"]], drug)

indx <- 3
plot(tbl.drug.test.dc.minus.10[[indx]]$predicted[,1], tbl.drug.test.dc.minus.10[[indx]]$actual,
     main = paste0("minus 10 corr: ", cor(tbl.drug.test.dc.minus.10[[indx]]$predicted[,1], tbl.drug.test.dc.minus.10[[indx]]$actual)))
plot(tbl.drug.test.dc.minus.5[[indx]]$predicted[,1], tbl.drug.test.dc.minus.5[[indx]]$actual, 
     main = paste0("minus 5 corr: ", cor(tbl.drug.test.dc.minus.5[[indx]]$predicted[,1], tbl.drug.test.dc.minus.5[[indx]]$actual)))
plot(tbl.drug.test.dc[[indx]]$predicted[,1], tbl.drug.test.dc[[indx]]$actual, 
     main = paste0("corr: ", cor(tbl.drug.test.dc[[indx]]$predicted[,1], tbl.drug.test.dc[[indx]]$actual)))

plot(density(as.numeric(subset(tbl.full.test.dc, model == "lasso" & metric == "spearman" & train.drug == drug)$val), na.rm = TRUE))
lines(density(as.numeric(subset(tbl.full.test.dc.minus.5, model == "lasso" & metric == "spearman" & train.drug == drug)$val), na.rm = TRUE))
lines(density(as.numeric(subset(tbl.full.test.dc.minus.10, model == "lasso" & metric == "spearman" & train.drug == drug)$val), na.rm = TRUE))

median(as.numeric(subset(tbl.full.test.dc, model == "lasso" & metric == "spearman" & train.drug == drug)$val), na.rm = TRUE)
median(as.numeric(subset(tbl.full.test.dc.minus.5, model == "lasso" & metric == "spearman" & train.drug == drug)$val), na.rm = TRUE)
median(as.numeric(subset(tbl.full.test.dc.minus.10, model == "lasso" & metric == "spearman" & train.drug == drug)$val), na.rm = TRUE)

subset(tbl.merge, train.drug == drug)
} # end if(FALSE)
