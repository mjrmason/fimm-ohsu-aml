...
scatterplot <- function(ds1, gene1, ds2, gene2, save.to.pdf = TRUE) {

  par(pty = "s")  
  foo <- extract.fit(res.aucs[[1]][["all.fits"]][[ds1]][["gene"]], train.drug = gene1, alpha = 0, model = "glmnet")
  bar <- extract.fit(res.aucs[[1]][["all.fits"]][[ds2]][["gene"]], train.drug = gene2, alpha = 0, model = "glmnet")

  tmp <- coefficients(foo, s = "lambda.min")
  c1 <- as.vector(tmp)
  names(c1) <- rownames(tmp)
  c1 <- c1[!grepl(pattern="Intercept", names(c1))]
  
  tmp <- coefficients(bar, s = "lambda.min")
  c2 <- as.vector(tmp)
  names(c2) <- rownames(tmp)
  c2 <- c2[!grepl(pattern="Intercept", names(c2))]
  c2 <- c2[names(c1)]
  if(save.to.pdf) {
    pdf(paste0(ds1, "-", gene1, "-vs-", ds2, "-", gene2, "-scatter.pdf"))
  }
  x = c1
  y = c2
  all = c(x,y)
  range = c(min(all), max(all))
##  plot(c1, c2, xlab = paste0(ds1, " ", gene1, " ridge coefficients"), ylab = paste0(ds2, " ", gene2, " ridge coefficients"), xlim=range, ylim=range)
  smoothScatter(c1, c2, xlab = paste0(ds1, " ", gene1, " ridge coefficients"), ylab = paste0(ds2, " ", gene2, " ridge coefficients"), xlim=range, ylim=range)
  reg <- lm(c2 ~ c1)
  abline(reg, col = "blue")
  if(save.to.pdf) {
    d <- dev.off()
