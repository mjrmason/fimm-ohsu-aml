...
rdata.file <- ".Rdata.validate.112217.one.ohsu.random"

## A string that will be included in any output files.
file.prefix <- "fimm-ohsu-validate-trained-models-112217-one-ohsu-random"
...
for(nm in names(exprs)) {
  ...
  cat(paste0("Randomizing expr for ", nm, "\n"))
  indices <- sample.int(ncol(exprs[[nm]]))
  samples <- colnames(exprs[[nm]])
  exprs[[nm]] <- exprs[[nm]][, indices]
  colnames(exprs[[nm]]) <- samples
    },
    "fimm" = {
    },
    "ctrp" = {
