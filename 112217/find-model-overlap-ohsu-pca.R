...
data.sets.to.analyze <- "ohsu"
file.prefix <- "fimm-ohsu-overlap-120717-ohsu-pca"
rdata.file <- ".Rdata.overlap.ohsu.pca.120717"
...
if(ds %in% data.sets.to.analyze) {
  ...
  ## Use PCs as features.
  ## foo <- expr.filt.matrices[[ds]][common.genes,]
  ## save(foo, file="ohsu-expr.Rd")
  ## q(status=0)
  pr <- prcomp(t(expr.filt.matrices[[ds]][common.genes,]), center=TRUE, scale=TRUE)
  percent.var <- (pr$sdev^2)/sum(pr$sdev^2)
  cutoff <- 1.00
  indx <- which(cumsum(percent.var) >= cutoff)[1]

  indices <- 2:length(percent.var)  
##      6 1
##      1 2
##      1 3
##      2 4
  indices <- 3:length(percent.var)  
##      9 1
##      1 2
  indices <- 1:length(percent.var)  
##      5 1
##      6 2
##      1 3
##      1 4
##      1 6
  ...
  indx <- which(cumsum(percent.var) >= cutoff)[1]
  indices <- 1:indx
## Using 21 PCs that cumulatively explain 0.756281637961232% of variance
##      9 1
##      1 11
##      4 2
##      4 3
...
##      1 8
  ...
  indx <- which(cumsum(percent.var) >= cutoff)[1]
  indices <- 1:indx
## Using 7 PCs that cumulatively explain 0.53214929752308% of variance
##     17 1
##      8 2
##      7 3
##      1 4
...
##      3 5
  ...
  cat(paste0("Using ", length(indices), " PCs that cumulatively explain ", sum(percent.var[indices]), "% of variance\n"))
  mat <- expr.filt.matrices[[ds]][common.genes, ] 
##     30 0
##      7 1
##      3 2
##      1 3
...
for(nm in names(lst)) {
  ...
  all.cross <- subset(lst[[nm]], train.set.1 == "ohsu.set1" & train.set.2 == "ohsu.set2")
  ...
  main <- "OHSU (train) vs OHSU (train)"
  cat(paste0("table: ", main, "\n"))
  print(tbl)
  pdf(paste0(file.prefix, "-ohsu-vs-ohsu-", nm , "-feature-overlap.pdf"))
  plot.table(tbl, main = main)
  d <- dev.off()

  if(grepl(x=nm, pattern="lasso")) {  
    s <- subset(all.cross, n.1 > 0 & n.2 > 0)
    tmp <- s[,c("FIMM_DRUG_NAME", "train.set.1", "n.1", "train.set.2", "n.2", "gene.set.1", "n.both", "pval")]
