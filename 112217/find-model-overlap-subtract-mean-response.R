...
file.prefix <- "fimm-ohsu-overlap-subtract-mean-response-120617"
validate.prefix <- "fimm-ohsu-validate-trained-models-112217-one-ohsu-no-filtering-subtract-mean-response"
rdata.file <- ".Rdata.overlap.subtract.mean.response.120617"
...
if(do.downsampling) {
  ...
  for(i in 1:length(seeds)) {
    ...
    res.aucs[[i]] <- train.and.test.crossproduct(gene.sets = gene.sets, drug.name.tbl = drug.name.tbl, train.set.names = train.set.names, train.dss.args = train.dss.args, 
                                           ...
                                           test.genomic.args = test.genomic.args, test.clinical.args = test.clinical.args, test.common.drugs = test.common.drugs,
                                           test.drug.cols = test.drug.cols, test.patient.cols = test.patient.cols, test.response.cols = test.response.cols, 
                                           seed = seed, use.rf = use.rf, use.svm = FALSE, use.mean = FALSE, num.processes = num.processes, alphas = glmnet.alphas, include.mean.response.as.feature = FALSE,
                                           subtract.mean.response = TRUE)
  }
}
