...
file.prefix <- "fimm-ohsu-overlap-subtract-mean-response-120817"
rdata.file <- ".Rdata.overlap.subtract.mean.response.120817"
...
if(do.downsampling) {
  ...
  for(i in 1:length(seeds)) {
    ...
    res.aucs[[i]] <- train.and.test.crossproduct(gene.sets = gene.sets, drug.name.tbl = drug.name.tbl, train.set.names = train.set.names, train.dss.args = train.dss.args, 
                                           ...
                                           include.mean.response.as.feature = FALSE, subtract.mean.response = TRUE)
  }
}
